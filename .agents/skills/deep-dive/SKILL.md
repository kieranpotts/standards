...
- **The tier to work — OPTIONAL.** Which tier of an existing plan to
  remediate. If not specified, take the lowest-numbered tier that is still open.
...
- `src/modules/ROOT/partials/<NNN>/TODO.md` MUST exist, MUST name the
  ...
  as a flat checklist bullet under its category heading.
...
8.  Verify mechanically before reporting a tier complete. Each check below
    ...
    - Count characters, not bytes. `awk 'length>80'` counts bytes, and an em
      ...
      wider than it is. So, use Python:
...
- Where a finding has two defensible resolutions, you MUST put it to the user
  ...
  apply it. The user will review your changes from the Git working tree.
...
- Renaming or renumbering a file MUST use `git mv`, so Git history follows the
...
- The standard is a stub, holding only a heading and a placeholder. There is
  nothing to review. Report this and stop.

- The plan's own claims have gone stale. `TODO.md` records what was true when
  it was written. Re-verify an item's premise before acting on it. A note saying
  a retrofit "requires renumbering identifiers" may be wrong by the time you
  reach it.
