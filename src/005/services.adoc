...
The modular monolith pattern, described in the section on *link:./vertical-slices.adoc[vertical slices]*, provides a natural stepping stone. Modules can be designed with clear boundaries and indirect communication from the outset, then extracted into independent services incrementally once their interfaces have stabilized.
