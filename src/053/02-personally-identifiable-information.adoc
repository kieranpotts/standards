...
For non-production systems, synthetic (dummy) data MUST be used in place of real PII. Alternatively, a hash of real PII MAY be used, as long as a strong cipher is used (making the hash irreversible without the cipher) and the cipher is handled as a secret – see *link:../052/README.adoc[TS-52: Security and Secrets Management]*.
