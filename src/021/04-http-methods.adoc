...
****
The `QUERY` method is a recent addition to the HTTP standards (as of mid-2026),
and support for it is not yet universal across HTTP clients, servers, proxies,
gateways, and CDNs. Until tooling support matures, `QUERY` SHOULD be used only
in environments where end-to-end support along the full request path can be
guaranteed, such as private APIs on internal networks. It is NOT RECOMMENDED for
public-facing APIs where the client's network stack and any intermediary
...
patterns. The behavior of the HTTP methods, listed above, SHOULD be identical
