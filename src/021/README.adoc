...
See also *link:../022/README.adoc[TS-22: Webhooks]*, which covers a specific pattern implemented in some public HTTP APIs to support asynchronous event notifications to clients.
...
HTTP APIs MUST use https://semver.org/[Semantic Versioning], as specified in *link:../011/README.adoc[TS-11: Versioning]*. However, only the major version number needs to be exposed in the URL schema.
