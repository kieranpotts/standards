...
See also *link:../016a/README.adoc[TS-16a: Webhooks]*, which covers a specific pattern implemented in some public HTTP APIs to support asynchronous event notifications to clients.
...
HTTP APIs MUST use https://semver.org/[Semantic Versioning], as specified in *link:../005/README.adoc[TS-5: Versioning]*. However, only the major version number needs to be exposed in the URL schema.
...
Health check responses MUST use the JSON format with the media type `application/health+json`.
...
* `warn`: The API is operational but experiencing degraded performance or non-critical issues.
...
The HTTP response status code MUST correspond to the value of the `status` field:

* For `pass` and `warn` statuses: HTTP response codes in the `2xx` or `3xx` range MUST be used (typically `200 OK`).

* For `fail` status: HTTP response codes in the `4xx` or `5xx` range MUST be used (typically `503 Service Unavailable`).

=== Caching

Health check responses SHOULD be cacheable. The server SHOULD provide guidance on appropriate caching using the `Cache-Control` header. A reasonable default cache lifetime is 1 hour, but this MAY be adjusted based on the specific requirements of the service.
...
It is RECOMMENDED that `ResourceItem` attributes be composed from a consistent set of common types, defined separately using JSON Schema. See *link:../018/README.adoc[TS-18: JSON Schema]* for further guidance on best practices for defining libraries of common types using JSON Schema.
