...
****
Richard I. Cook's seminal paper https://how.complexsystems.fail/[_How Complex Systems Fail_] (1998) provides empirical grounding for why simplicity in software design matters. Drawing on research in high-stakes domains like healthcare and aviation, Cook observes that complex systems inevitably run with latent failures present within them – flaws that are individually insufficient to cause disaster but which, in combination, create the conditions for catastrophic failure. A corollary is that complex systems routinely operate in a degraded state, held together by human adaptation rather than by structural soundness.

Cook also notes that change introduces new failure modes. Improvements intended to increase reliability often create new, less-understood pathways to catastrophic failure. And safety, he argues, is not a property of any individual component but an emergent characteristic of the system as a whole.

The key ideas in the paper are:

1. *Complex systems are inherently hazardous* — risk cannot be fully eliminated, only defended against.

2. *Multiple layers of defense* prevent most failures, but catastrophe still lurks beneath.

3. *Catastrophe requires multiple simultaneous failures* — no single point of failure causes disaster alone.

4. *Latent failures are always present* — complex systems cannot run without flaws; they're kept operational despite them.

5. *Systems run in degraded mode* — the baseline state is broken, held together by redundancy and human adaptation.

6. *Catastrophe is always just around the corner* — failure potential is intrinsic and irreducible.

7. *"Root cause" is a myth* — accidents have multiple necessary contributors; identifying a single cause reflects social/cultural blame-seeking, not technical reality.

8. *Hindsight bias distorts post-accident analysis* — knowing the outcome makes prior warning signs seem more obvious than they were at the time.

9. *Operators play dual roles* — simultaneously producing output and defending against failure.

10. *All practitioner actions are gambles* — decisions are made under uncertainty; successful outcomes are equally the result of luck.

11. *Ambiguity is resolved at the sharp end* — front-line workers absorb organizational contradictions (cost vs. safety, speed vs. caution).

12. *Humans are the adaptable element* — practitioners continuously reshape the system to maintain operations.

13. *Expertise is always in flux* — skills and knowledge are unevenly distributed and constantly being transferred.

14. *Change introduces new failure modes* — improvements to reliability often create new, less-understood catastrophic failure paths.

15. *Post-accident remedies focused on "human error" are mostly ineffective* — they increase coupling and complexity without addressing root causes.

16. *Safety is an emergent system property* — it is not a component or department.

17. *People continuously create safety* — failure-free operation is an active, moment-to-moment achievement, not a passive state.

18. *Failure-free operations require experience with failure* — operators must know the "edge of the envelope" to avoid crossing it.

These observations reinforce the case for simplicity. Every increase in coupling and structural complexity adds to the number of latent failure opportunities and makes accident trajectories harder to detect and block. Simple systems – with loosely coupled components and clear, consistent structure – are not just easier to change, they are structurally safer.
****
