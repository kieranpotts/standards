...
Both produce the expected result, however the internal bold style is less reliable in preview mode in some AsciiDoc-compatible text editors. For this reason, the outer `*` characters are recommended for internal links.

It is RECOMMENDED to distinguish internal from external links by making only internal links bold.
