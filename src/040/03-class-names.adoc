...
* It modifies the default presentation of an individual HTML element, a component,
  or a layout section.
...
cases be generated entirely by client-side scripting (see <<components,Components>>).
...
presentation of certain HTML elements, a certain component, or a particular layout
section. Modifiers MAY also be added to a document dynamically by client-side
JavaScript, to act as targets for CSS animations and transitions (see
<<modifiers,Modifiers>>).

Every class MUST represent the name of a layout section, the namespace for a
component, or a modifier of something's default presentation. A single class MUST
NOT fuse these separate concerns.

== Naming conventions

Layout classes, component classes, and modifier classes each use a distinct naming
convention, so that a class's role is identifiable from its name alone:
...
Modifiers that belong to a particular component or layout section MUST be prefixed
with the lower-case name of that component or section (eg. `logo-homepage`,
`contextmenu-open`).
