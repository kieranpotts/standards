...
Long lines SHOULD be "soft wrapped" at 80 characters — that is, continued on the next line without a trailing backslash or trailing spaces. CommonMark joins consecutive lines within a paragraph into a single line in the rendered output, so soft wrapping does not affect how the document renders.
...
Lines MUST NOT exceed 160 characters, except where this cannot be achieved — for example, in tables or long URLs that cannot themselves be broken across lines.
