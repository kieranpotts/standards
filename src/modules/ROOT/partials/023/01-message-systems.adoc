...
* xref:garden::publish-subscribe-pattern.adoc[*Publish-subscribe pattern*]: A one-to-many message distribution pattern where messages are published to a topic and multiple subscribers can receive them. This is supported by MQTT, AMQP, Kafka, and Redis.
...
* xref:garden::scalability.adoc[*Scalability*]: Kafka is designed for massive scale, while RabbitMQ and NATS are for moderate scale.

* xref:garden::complexity.adoc[*Complexity*]: Kafka and AMQP are feature rich but more complex for it, while MQTT and NATS focus on simplicity.
...
* xref:garden::grpc.adoc[*gRPC*]: A high-performance RPC framework developed by Google. It uses HTTP/2 as the transport protocol and Protocol Buffers for serialization. gRPC supports bi-directional streaming and is well-suited for microservices, polyglot environments, and low-latency APIs.
