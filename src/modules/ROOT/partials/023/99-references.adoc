...
* Brandur Leppka (2017). https://brandur.org/http-transactions[_Using Atomic Transactions to Power an Idempotent API_]. — The source for the enqueuer/staging-table framing of the transactional outbox pattern in <<transactional-outbox,Transactional outbox>>.

* Brandur Leppka (2017). https://brandur.org/job-drain[_Transactionally Staged Job Drains in Postgres_]. — The source for the single-enqueuer, read-then-delete consistency, batching/backoff, and in-database-queue scaling guidance in <<transactional-outbox,Transactional outbox>>.
