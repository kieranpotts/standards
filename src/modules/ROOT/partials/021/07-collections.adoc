...
Query parameters SHOULD be used only to restrict, search, or filter a collection. A resource identifier SHOULD NOT be used as a query parameter value to select a single resource out of a collection; the identifier belongs in the URL path instead, per the *Resources and collections* section of the *URLs* section, above.
...
It is often necessary for a client to filter a collection by more than one value of the same field, eg. to request resources whose `status` is either `open` or `closed`. Two patterns are RECOMMENDED for this, depending on context.
...
Where this pattern is used, the query parameter's documented name SHOULD be singular (`status`, not `statuses`), since each occurrence carries a single value.

Alternatively, a single query parameter MAY accept a comma-separated list of values, eg. `?statuses=open,closed`. This can be preferable where the number of values is potentially large and the 2,000-character practical limit on URL length is a concern. However, this pattern is NOT RECOMMENDED unless justified, because it requires bespoke parsing logic on both the client and server, and it is not universally supported by API tooling. Where this pattern is used:

* The query parameter's documented name SHOULD be plural (`statuses`), to signal to clients that comma-separated values are accepted.
...
*Homogeneous bulk operations* – applying the same operation (eg. `create`) to many resources of the same type – are RECOMMENDED where this trade-off is worthwhile. *Heterogeneous bulk operations* – combining different operations, or different resource types, in a single call – are NOT RECOMMENDED, because they are difficult to make atomic, difficult to report errors for consistently, and difficult for clients to reason about. Where heterogeneous batching is genuinely required, consider adopting an established standard such as the OData Batch specification, rather than inventing a bespoke format.

A homogeneous bulk operation SHOULD be modelled as an action on the collection, accepting an `items` array in its request body:
...
Where an error needs to reference a specific field within a specific item of the request, the error's `field` JSON Pointer SHOULD include the item's index, eg. `/items/1/currency_code`. Where the request items do not have a stable index – for example, where a client-supplied identifying attribute is more meaningful to the client than positional order – an attribute-filter form MAY be used instead, eg. `/items/@account_number=='2097094104180012047'/address_id`. An API MUST document which of the two forms it uses, and MUST use that form consistently across all of its bulk operations.

A bulk operation MUST document whether it is *atomic* (all items succeed, or none are applied) or *partial* (each item succeeds or fails independently). An atomic bulk operation SHOULD use the normal single-resource status codes, since the whole operation either succeeds or fails as one. A partial bulk operation SHOULD normally respond with `200 OK` for the operation as a whole, with the per-item outcomes conveyed in the response body as shown above, since no single top-level status code can represent a mix of per-item successes and failures. Where a bulk operation is processed asynchronously, `202 Accepted` SHOULD be used instead.

Bulk operations SHOULD be atomic wherever this is practical for the underlying system to implement. Where an operation is not atomic, this MUST be documented, so that clients know to check the per-item results rather than assuming a `2xx` response means every item succeeded.

=== Bulk update and replace

Where a client needs to `PUT` or `PATCH` many resources in a single call, the `bulkCreate`-style action described above is not directly reusable, because there is no single collection URL to target the update at. Instead, model the bulk request itself as a first-class, uniquely identifiable *batch resource*.

A bulk-create action MAY return the identifier of the batch it created, in addition to the created resources' own identifiers:
...
Subsequent bulk updates or replacements are then addressed to that batch, and act on its constituent items the same way a single-resource `PUT` or `PATCH` acts on one resource:
...
As with bulk create, every effort SHOULD be made to make a bulk update or replace atomic. Where atomicity is not practical, the response SHOULD follow the same partial-response shape as bulk create, described above.
