...
Where the operation resource supports more than `GET` – eg. the `DELETE` cancellation described in *Operations resource*, below – the response SHOULD include a link for each supported operation, not only `self`, so that clients can discover what is possible without hard-coding assumptions about which methods a given operation supports:
...
Where the second option is used and the operation creates a new resource, once the operation completes, the status endpoint SHOULD respond with `303 See Other` and a `Location` header pointing at the newly-created resource's URL, rather than continuing to return the operation's own status representation. This gives the client a single, explicit signal that processing has finished and where the result now lives, without requiring it to infer completion from the status body's contents.
...
Where an API implements a substantial amount of asynchronous processing, it is RECOMMENDED to expose a dedicated `/operations` collection, rather than only ad hoc per-request status URLs, so that operation status is modelled consistently and clients have one place to discover in-flight and past work.
...
`GET /v\{major}/operations` SHOULD support pagination, sorting, and filtering per the *Collections* section. The default sort order SHOULD list operations by state, in the order `not_started`, `running`, `succeeded`, `failed`, and by creation time within each state.
...
* `percent_complete`: An estimate of progress, where the underlying processing can support one. This field MAY be omitted where no meaningful estimate is available.

An operation resource MAY support `DELETE`, to allow a client to request cancellation of an in-progress operation. Where implemented, `DELETE` MUST be idempotent, per the *Safeness and idempotency* section – repeating a cancellation request against an already-cancelled or already-completed operation MUST NOT be treated as an error.

A server MAY retain a *tombstone* – a minimal record that the operation existed and how it concluded – for some period after an operation completes, rather than deleting the record immediately, so that a client that was disconnected during processing can still discover the outcome. This retention period MUST be documented per API, since the appropriate duration depends on how long a client might reasonably remain disconnected.

It is RECOMMENDED that all HTTP APIs that implement asynchronous processing also support a single webhook that clients may optionally implement to receive push notifications of any asynchronously-updated resources, or the results of any asynchronously-processed actions. This offers a third option for clients to keep their state synchronized with server changes – whether triggered by asynchronous operations or even by other clients. See xref:022.adoc[TS-22] for further guidelines on implementing this option.
