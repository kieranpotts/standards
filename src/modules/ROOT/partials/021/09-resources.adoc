...
* Where a sub-resource identifier is used (see *Sub-resources and sub-collections*), it MUST be scoped for lookup purposes within its parent resource only. For example, given `/users/1234/linked-accounts/ABCD`, account `ABCD` MUST NOT be returned unless it is actually linked to user `1234`, even if an account with that identifier exists elsewhere in the system.

* A URL path MUST NOT contain two resource identifiers in immediate succession, eg. `/payments/12345/102030`, since this is ambiguous as to whether the second segment is a sub-resource identifier or something else. Interpose a named sub-resource segment between them, per the templates above.

* Resource identifiers SHOULD be restricted to ASCII characters. Where an identifier must be included in a URL and contains any character outside of the URI-unreserved set, it MUST be percent-encoded, as MUST any UTF-8 characters in query parameter values.
...
Where a resource may itself carry large or binary content, eg. one of the files described in *File uploads*, below, the corresponding `GET` endpoint MAY support ranged retrieval, so that a client can fetch the content in chunks rather than downloading it in a single response.

Where supported, the response to an ordinary `GET` MUST include an `Accept-Ranges: bytes` header, so that clients can discover the capability. A client MAY then issue a `HEAD` request first to discover the resource's total size without transferring its content, followed by one or more ranged `GET` requests using the `Range` header:
...
The server responds with `206 Partial Content`, a `Content-Range` header identifying which part of the resource was returned, and a `Content-Length` reflecting only the bytes in this response, not the resource's total size:
...
The plain partial-representation form of `PATCH` described above is equivalent to https://datatracker.ietf.org/doc/html/rfc7396[JSON Merge Patch], and where implemented as such SHOULD use the `application/merge-patch+json` media type. Because a `null` value in a JSON Merge Patch document means "delete this field", merge patch is not suitable for a resource that can legitimately hold an explicit `null` value in one of its own fields – there is no way to distinguish "set this field to `null`" from "delete this field" in the patch document itself.

Alternatively, standards such as https://datatracker.ietf.org/doc/html/rfc6902[JSON Patch] MAY be implemented for `PATCH` requests, using the `application/json-patch+json` media type. Rather than sending a partial representation of the resource, clients instead send a list of operations to be made on particular members or fields of the resource. JSON Patch does not share Merge Patch's `null` ambiguity, and it additionally allows operations to be sequenced, which matters where the order changes MUST be applied in.
...
An API that supports `PATCH` MUST document which of these formats (plain partial representation, JSON Merge Patch, or JSON Patch) it accepts, via the request's `Content-Type` header. A `PATCH` request whose `Content-Type` names a format the endpoint does not support MUST receive `415 Unsupported Media Type`, per *Headers*.
...
*Standalone file upload.* Where the file is itself the primary subject of the request, eg. uploading a profile photo, it SHOULD be submitted to a dedicated URL using the `multipart/form-data` media type, per https://www.ietf.org/rfc/rfc2388.txt[RFC 2388]. The request body SHOULD carry a part for the binary file content, identified by a `Content-Disposition` header with both `name` and `filename` parameters, and MAY carry a further part for accompanying metadata.
...
The response MUST include the uploaded file's metadata, which clients then reference in subsequent requests, rather than re-submitting the file's content each time. At minimum, this SHOULD include an `id`, `create_time`, `size` (in bytes), `url` (from which the file's content can subsequently be retrieved), and `type` (its media type).

*File upload as an attachment to structured data.* Where a file needs to be submitted together with structured JSON data in the same request, eg. uploading a receipt alongside an expense claim's other fields, the request SHOULD use `multipart/mixed` or `multipart/related`, per https://tools.ietf.org/html/rfc2387[RFC 2387], with one part carrying the JSON body and one or more further parts carrying the file content.
