...
The string representation of an enumeration value MAY be used as a sub-resource identifier, where the enumeration has a fixed, well-known set of members. For example, a `credit-cards/\{credit_card_id}/limits/monthly` endpoint could use the enumeration value `monthly` directly as the `\{sub_resource_id}`, rather than requiring an opaque identifier to be looked up first.
