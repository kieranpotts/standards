...
Single resources are typically discovered through a collection, and are
identified by a unique identifier. When reading single resources, a more
detailed representation of the resource MAY be returned than the default,
minimized representations included in collections.

A resource's unique identifier SHOULD be unique to all resources of all types,
not only resources of the same type or in the same collection. UUIDs are
RECOMMENDED for this purpose, as each generated UUID is more-or-less guaranteed
to be unique globally.

Identifiers for sensitive data SHOULD be non-sequential, and preferably
non-numeric. In scenarios where this data might be used as a subordinate to
other data, immutable string identifiers SHOULD be used for readability and
debugging.

The following additional rules apply to the design of resource identifiers:

* A resource identifier's lifecycle MUST be owned by the resource's own domain
  model. Database-generated sequence numbers MUST NOT be used as resource
  identifiers, since doing so leaks an internal implementation detail (the
  choice of database and its auto-increment behavior) into the public contract,
  and typically also makes identifiers sequential and guessable. Where a UUID is
  not suitable, eg. for performance or storage reasons, an HMAC-based hashed
  identifier is an acceptable alternative.
...
* Where a sub-resource identifier is used (see
  *Sub-resources and sub-collections*), it MUST be scoped for lookup purposes
  ...
  `/users/1234/linked-accounts/ABCD`, account `ABCD` MUST NOT be returned unless
  it is actually linked to user `1234`, even if an account with that identifier
  exists elsewhere in the system.

* A URL path MUST NOT contain two resource identifiers in immediate succession,
  eg. `/payments/12345/102030`, since this is ambiguous as to whether the second
  segment is a sub-resource identifier or something else. Interpose a named
  sub-resource segment between them, per the templates above.

* Resource identifiers SHOULD be restricted to ASCII characters. Where an
  identifier must be included in a URL and contains any character outside of the
  URI-unreserved set, it MUST be percent-encoded, as MUST any UTF-8 characters
  in query parameter values.
...
Where a resource may itself carry large or binary content, eg. one of the
files described in *File uploads*, below, the corresponding `GET` endpoint MAY
support ranged retrieval, so that a client can fetch the content in chunks
rather than downloading it in a single response.

Where supported, the response to an ordinary `GET` MUST include an
`Accept-Ranges: bytes` header, so that clients can discover the capability. A
client MAY then issue a `HEAD` request first to discover the resource's total
size without transferring its content, followed by one or more ranged `GET`
requests using the `Range` header:
...
The server responds with `206 Partial Content`, a `Content-Range` header
identifying which part of the resource was returned, and a `Content-Length`
reflecting only the bytes in this response, not the resource's total size:
...
The plain partial-representation form of `PATCH` described above is equivalent
to https://datatracker.ietf.org/doc/html/rfc7396[JSON Merge Patch], and where
implemented as such SHOULD use the `application/merge-patch+json` media type.
Because a `null` value in a JSON Merge Patch document means "delete this
field", merge patch is not suitable for a resource that can legitimately hold
an explicit `null` value in one of its own fields – there is no way to
distinguish "set this field to `null`" from "delete this field" in the patch
document itself.

Alternatively, standards such as
https://datatracker.ietf.org/doc/html/rfc6902[JSON Patch] MAY be implemented for
`PATCH` requests, using the `application/json-patch+json` media type. Rather
than sending a partial representation of the resource, clients instead send a
list of operations to be made on particular members or fields of the
resource. JSON Patch does not share Merge Patch's `null` ambiguity, and it
additionally allows operations to be sequenced, which matters where the order
...
https://tools.ietf.org/html/rfc6901[JSON Pointer] that references the location
...
The supported operations are "add", "remove", "replace", "move", "copy", and
"test". To support partial updates to fixed-schema resources, APIs need to
support only "add", "remove", and "replace" operations.

An API that supports `PATCH` MUST document which of these formats (plain
partial representation, JSON Merge Patch, or JSON Patch) it accepts, via the
request's `Content-Type` header. A `PATCH` request whose `Content-Type` names
a format the endpoint does not support MUST receive
`415 Unsupported Media Type`, per *Headers*.
...
There may be use cases where a client expects resources to exist but they
...
_not_ require DELETE requests to be accompanied by a payload. This is an
...
* Using `POST` to create a resource but have the server create an identifier for
  it.
* Using `PUT` to create a full resource, including a unique identifier that is
...
`PUT` operations are idempotent by default, because the request payload has a
...
unique identifier for the _request_ message (eg. `request_id`). The server will
...
The number of required fields SHOULD be minimized as much as possible. Implement
as many default/fallback values as can reasonably be applied for each business
case.
...
Where a resource needs to carry binary content, eg. an image, a document, or an
audio file, that content SHOULD NOT be embedded in a JSON payload as a
Base64-encoded string. Base64 encoding inflates the size of the content by
roughly a third, forces the entire file to be held in memory as a single JSON
string on both client and server, and defeats HTTP-level features such as
streaming and range requests. Two patterns are RECOMMENDED instead, depending on
whether the file exists independently of other resource data.
...
https://www.ietf.org/rfc/rfc2388.txt[RFC 2388]. The request body SHOULD carry a
part for the binary file content, identified by a `Content-Disposition` header
with both `name` and `filename` parameters, and MAY carry a further part for
...
The response MUST include the uploaded file's metadata, which clients then
reference in subsequent requests, rather than re-submitting the file's content
each time. At minimum, this SHOULD include an `id`, `create_time`, `size` (in
bytes), `url` (from which the file's content can subsequently be retrieved),
and `type` (its media type).
...
https://tools.ietf.org/html/rfc2387[RFC 2387], with one part carrying the JSON
