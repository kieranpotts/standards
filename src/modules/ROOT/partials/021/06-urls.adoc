...
This section covers URL design as it applies to HTTP APIs specifically — versioning, namespaces, and resource hierarchies. For general URL design principles that apply to all HTTP services (path casing, query strings, fragments, permanence), see xref:063.adoc[TS-63].
...
RFC 7230 defines no length limit on the HTTP request line, but in practice client libraries, proxies, and server frameworks all impose their own limits. A URL, including its query string, SHOULD be kept under 2048 characters, to stay compatible with the most restrictive of the widely-used stacks.

Where a request's target URL exceeds a server's configured limit and cannot be parsed, the server MUST respond with `414 URI Too Long`. An endpoint whose normal, valid usage risks approaching the length limit, eg. one accepting a long comma-separated list of filter values (see the *Filtering by multiple values* section of *Collections*), SHOULD offer a `POST`-based alternative – see the *Complex search actions* section of *Actions* – rather than requiring clients to work around the limit.
...
HTTP APIs MUST use https://semver.org/[Semantic Versioning], as specified in xref:011.adoc[TS-11]. However, only the major version number needs to be exposed in the URL schema.
...
Query parameters SHOULD NOT be used on single-resource endpoints (eg. `GET /v1/vault/credit-cards/\{credit_card_id}`). Query parameters exist to restrict, search, filter, or sort a *collection*; a single-resource endpoint already identifies exactly one resource via the URL path, so there is nothing left for a query parameter to narrow down. See the *Filtering* section of the *Collections* section, below, for the collection-scoped rule this complements.
