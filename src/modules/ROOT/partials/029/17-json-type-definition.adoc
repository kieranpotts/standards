...
The `properties` form defaults to rejecting any instance property not explicitly listed — the equivalent of JSON Schema's `additionalProperties: false` (see <<applicator-keywords,Applicator keywords>>) — with no per-schema way to opt out. This is a sharper default than JSON Schema's, where `additionalProperties` defaults to permissive.
