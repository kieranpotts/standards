...
`unevaluatedProperties` and `unevaluatedItems` constrain whatever an instance's properties or array elements were *not* already matched by any other applicator in the schema — including subschemas reached via `allOf`, `$ref`, or a conditional `if`/`then` branch (see <<conditional-validation,Conditional validation>>). This is a broader net than `additionalProperties` and `items` (see <<applicator-keywords,Applicator keywords>>), which only see the sibling keywords in their own schema object and are blind to properties matched by a composed subschema.
