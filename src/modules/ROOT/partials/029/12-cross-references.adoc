...
It is RECOMMENDED to use `$ref` to share common structures, keeping schemas modular and reusable. Prefer schema composition over inheritance — see <<applicator-keywords,Applicator keywords>> for the `allOf`/`anyOf`/`oneOf`/`not` keywords used to combine referenced subschemas. Designing your schema for modularity is how, like code, you can best manage growing complexity at scale.
