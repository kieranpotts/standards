...
* *Keep description and reasoning in their proper homes.* The specification says _what_ the system does, while the proposals say _why_ changes were made over time. Rationale SHOULD NOT be smuggled into the specification, and a proposal SHOULD NOT restate the specification — it SHOULD instead link to the artifacts it edits.
...
.Definition of Done
****
The DoR has a counterpart at the other end of the development task — the *Definition of Done (DoD)*. This defines the conditions under which work has ended. The DoR describes when work MAY begin. The DoD describes when it is complete. See xref:012.adoc[TS-12].
****
