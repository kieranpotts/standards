...
Acceptance criteria MUST be objectively verifiable. Each AC SHOULD resolve to a clear pass/fail condition, not a subjective judgment such as "user-friendly" or "reasonably fast". Each criterion SHOULD, if practical, be expressed in an <<executable-specifications,executable form>> such that its verification can be automated.
