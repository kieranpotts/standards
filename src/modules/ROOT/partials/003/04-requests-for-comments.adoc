...
An architectural decision record (ADR) captures a _single_ decision and its rationale. An RFC, as used in this standard, captures the _overall_ design of a system or change, which typically bundles many decisions.
...
A request for comments is, strictly, a _process_ — a document circulated for review — rather than a document type. In practice the terms are used interchangeably: many organizations call their design docs "RFCs" and run a lightweight review process around them. This standard uses "RFC" for the artifact and treats review as one phase of its lifecycle (see <<Review>>).
...
Where the design's central question is which of several candidate technologies to adopt, a prototype is most convincing when it tests the candidates against conditions close to what production will actually demand, rather than a synthetic benchmark or a happy-path demo.

* *Test against realistic workloads.* Query patterns, data volumes, and traffic shapes copied from production — or, where the system is new, the best available estimate of them — surface failure modes that an idealized benchmark hides. A candidate that performs well on a small, evenly distributed test dataset can degrade sharply under the skew and scale of real data.
* *Mirror live traffic where practical.* Replaying a sample of real production traffic against a candidate, or running it as a shadow deployment alongside the incumbent, projects cost and performance more reliably than an isolated load test. See xref:014.adoc[TS-14] for the testing methodology such an evaluation draws on.
* *Bound the evaluation.* An open-ended proof-of-concept tends to expand to fill the time available. Treat it as a scoped exercise with a fixed question to answer and a deadline, not an ongoing side project running alongside the RFC.
* *Record the result in the RFC, win or lose.* A prototype that ruled out a candidate is evidence for <<_alternatives_considered,Alternatives considered>>, not a wasted effort — it is what turns "we considered X" into a reasoned trade-off rather than an assertion.
...
The document moves from _facts_ to _direction_ to _decisions_. Context and scope are objective background. Goals and non-goals turn that background into a direction. The design proposes a solution. Alternatives considered justify why that solution was chosen over the others. Cross-cutting concerns close the loop on the obligations the design does not center on but must not ignore.
...
*System-context diagram.* For most systems, a diagram showing the system as part of its larger technical landscape is valuable. It lets readers contextualize the new design against an environment they already know. For diagram conventions, see xref:025.adoc[TS-25].
...
* *Security* — how the design affects the system's attack surface and how threats are mitigated. See xref:052.adoc[TS-52].
...
* xref:garden::compatibility.adoc[*Compatibility*] — impact on existing interfaces, clients, and data.
* xref:garden::operability.adoc[*Operability*] — deployment, rollback, capacity, and runbook implications.
...
The primary value of review is to fold the combined experience of the organization into the design before it is expensive to change. Its most consistent contribution is ensuring cross-cutting concerns — observability, security, privacy — are addressed (see <<_cross_cutting_concerns>>). The value of review is not that issues are discovered, but that they are discovered _early_. A review that finds a security gap before a line of code is written has paid for the whole review process.

=== Grounds for rejection

A well-researched RFC is not entitled to acceptance. Review exists to test the design against information the author did not have, and it MAY conclude that the design should not proceed. Valid grounds for rejection include:

* *Immaturity.* A candidate technology or approach central to the design is not yet stable, supported, or proven enough to bet production on, even where it is otherwise well suited to the problem.
* *Poor fit beyond the proposing team.* The design solves the immediate problem but does not generalize to how other teams will need to use or operate it — for example, an interface shaped around one consumer's needs, or an operational model incompatible with the organization's existing tooling.
* *A more important criterion surfacing during review.* Reviewers with context the author lacked may surface a constraint — a compliance requirement, a planned migration, a conflicting roadmap commitment — that outweighs the criteria the design was optimized against.
* *Real-world evaluation contradicting expectations.* Where a prototype (see <<evaluating-candidate-technologies,Evaluating candidate technologies>>) was conducted, its results override the original assumptions. A design whose central premise the prototype disproved should be rejected even if the document itself is otherwise sound.

A rejection SHOULD state which of these grounds applied, and SHOULD be as specific as the <<_alternatives_considered>> section it mirrors: "we rejected this because Y" is not enough on its own — the reader needs the trade-off or evidence that ruled it out. A rejected RFC is preserved, not discarded (see <<Immutability>>), so a clearly stated reason is what lets a future author avoid re-proposing the same design without addressing why it failed before.
