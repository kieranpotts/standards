...
Environment configuration MUST be treated as external input to an application, and therefore values MUST be validated and sanitized before use. Tools such as https://pkl-lang.org/[Pkl] can be helpful for defining configuration schemas and for validating environment configurations against the schemas.
