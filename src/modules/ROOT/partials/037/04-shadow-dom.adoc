...
Prefer Light DOM — regular, unencapsulated markup — for as long as it solves the problem. Attach a shadow root only once encapsulation is solving a real problem: a genuine style-collision risk, or a component that MUST be portable into pages whose styles cannot be predicted or controlled. Most in-application components, including the HTML web components described in <<html-web-components-vs-javascript-web-components,HTML web components vs. JavaScript web components>>, do not need it.
