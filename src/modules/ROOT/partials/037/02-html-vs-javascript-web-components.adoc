...
Neither example needs Shadow DOM to be a legitimate web component. Shadow DOM is one capability that custom elements MAY use, not a requirement for something to count as a web component — see <<choosing-an-encapsulation-strategy,Choosing an encapsulation strategy>> for when it earns its cost.
