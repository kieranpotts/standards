...
HTML is fault-tolerant by design: a browser renders any recognized markup it finds, and silently ignores tags it does not understand. This has a useful consequence for HTML web components (see <<html-web-components-vs-javascript-web-components,HTML web components vs. JavaScript web components>>): any HTML placed between a custom element's opening and closing tags renders automatically, exactly as written, if the component's script never loads.
