...
Prefer the following widely-supported event types; a "DOM"-prefixed event (`DOMAttrModified`, `DOMNodeInserted`) is legacy and MUST NOT be used — observe DOM mutations with `MutationObserver` instead (see <<observing-dom-mutations,Observing DOM mutations>>).
...
Restrict client-side HTTP requests to fetching and submitting *data*, not markup. A request MUST NOT return a partial HTML view to be spliced into the page — all of the markup a page can ever need SHOULD already be present in the document from the initial server render, per the progressive enhancement model described in xref:018.adoc[TS-18], "Performance optimization".
