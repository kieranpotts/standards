...
Where a page documents a language that itself uses angle brackets — Gherkin scenario outlines, for example — the two uses (the language's own syntax, and the placeholder convention in xref:026.adoc[TS-26]) are indistinguishable on the page. Say so explicitly in the surrounding prose rather than switching notation.
