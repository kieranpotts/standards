...
AsciiDoc also supports a curly-quote syntax for inline quoted text — `` "`quoted text`" `` — which renders as typographic opening and closing quotation marks. This MUST NOT be used. Quoted prose is written with plain, straight double quotes (`"quoted text"`) instead; see xref:026.adoc[TS-26] for the full quotation-marks rule. This is unrelated to air quotes above, which are a block-level construct for setting off a whole quoted passage rather than an inline typographic substitution.
