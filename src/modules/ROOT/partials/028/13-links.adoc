...
A URL may not display correctly when it contains characters such as underscores (`_`), carets (`^`), or double quotes (`"`). The following is an example of a URL containing characters that need special treatment. See https://asciidoctor.org/docs/user-manual/#complex-urls[Asciidoctor: Troubleshooting URLs] for help solving this.

```
https://www.owasp.org/index.php/Cross-site_Scripting_(XSS)
```

Every external URL used on a page — a reference-list entry or an inline prose link alike — MUST be declared as a document-level attribute near the top of the page and referenced by name, rather than inlined as a raw `https://...[text]` macro in the body:
...
The older `[[<id>]]` double-bracket syntax is equivalent for a block-level anchor and MAY still be encountered, but `[#<id>]` is preferred: it is the same shorthand used for roles (`.role`) and options (`%option`), so it composes with them in one attribute list (`[#id.role]`) where `[[id]]` cannot. `[[<id>]]` remains the only option for an *inline* anchor — a point marked in the middle of a sentence or list item rather than on a block of its own — since the `[#id]` shorthand only attaches to a block.
...
For guidance on writing effective link text, see xref:026.adoc[TS-26].
...
Both produce the expected result, but the inner form — bold applied tightly around the link text, inside the macro's brackets — MUST be used. Bolding the whole link construct, brackets included, is NOT RECOMMENDED: the asterisks then sit outside the part a reader would select or edit, and the convention diverges from Markdown, which has no equivalent for wrapping a link construct in bold (see xref:027.adoc[TS-27]).
