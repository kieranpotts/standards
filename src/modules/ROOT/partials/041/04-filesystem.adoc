...
NOTE: Compilers do not distinguish between `.js` and `.jsx` (or between `.ts` and `.tsx`) — the extension is a convention for the benefit of people reading the code, not a requirement of the toolchain. Using the JSX-specific extension makes it immediately clear which files contain JSX syntax.
