...
as props. This pattern means that consumers could pass invalid props, which are
not supported by the abstracted native element. However, since HTML will just
ignore invalid attributes, this will not cause any runtime issues. It's a good
trade-off for the increased flexibility.
...
native elements and/or other components. In this case, the props are the part of
the component's declarative API, and should be explicitly defined.
