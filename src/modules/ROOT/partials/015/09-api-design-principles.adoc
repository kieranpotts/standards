...
* *Keep the barrier to entry low.* A new consumer SHOULD be able to make a first successful call with minimal setup and minimal reading. Simplicity is a usability property of an API in the same way it is a usability property of a GUI — see <<keep-it-tidy,Keep it tidy>>.
...
For the design of HTTP-specific API surfaces — resource modeling, status codes, versioning, pagination — see xref:021.adoc[TS-21]. This section covers general API usability principles that apply regardless of transport or protocol.
