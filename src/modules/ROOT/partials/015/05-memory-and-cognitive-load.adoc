...
See xref:025.adoc[TS-25] and xref:026.adoc[TS-26] for how to structure and write documentation itself; this section covers only where and how it should surface within an interface.
...
* *Design to match users' mental models.* A user's mental model is their internal understanding of how the system works, built from prior experience with similar systems. The closer the interface matches that model, the less the user has to learn. User research — interviews, personas, journey maps, empathy maps — narrows the gap between the designer's model of the system and the user's model of it; see <<understand-context,Understand context>> for more on grounding design decisions in research about real users.

This section covers consistency with the outside world. For consistency *within* an interface, see <<keep-it-tidy,Keep it tidy>> and the note on when to diverge from uniformity, below.
