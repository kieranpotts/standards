...
* *Vary tone by severity.* A minor, easily-corrected error (a missing form field) can use a casual, light tone. A severe error (data loss, a failed payment) SHOULD use a sincere, direct tone. See <<voice-and-tone,Voice and tone>> for how tone relates to the interface's overall voice.
