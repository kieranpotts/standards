...
* *Give clear feedback when input is normalized or rejected.* Where the interface accepts a loose input and normalizes it, show the user the normalized value, so they can confirm it was interpreted correctly. Where input must be rejected, explain why — see <<error-messages,Error messages>>.
...
* *Watch for unintentional discrimination.* A design built around the designer's own habits, abilities, and context of use can systematically exclude users whose habits, abilities, or context differ — see <<understand-context,Understand context>>. Awareness of common cognitive biases helps a designer recognize when a decision is being driven by an unexamined assumption rather than by evidence about real users.
