...
Where multiple issues need to be completed to deliver value to one or more stakeholder groups, the issues SHOULD be grouped under a single epic. Epics serve as organizational containers for large or complex projects, such as major new user-facing features or re-platforming of a legacy system.

Epics SHOULD NOT be used as generic containers for related issues. Epics are intended for grouping dependent work items that together compose the delivery of a complete unit of functionality or behavior.
...
* **Small feature.** A single user-facing feature broken into multiple implementation tasks (eg. "Add dark mode support")
