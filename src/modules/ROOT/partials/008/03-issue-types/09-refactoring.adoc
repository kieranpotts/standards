...
Including refactoring work in the scope of other changes keeps the internal quality consistent over time. It also gives an accurate measure of the _true_ cost of changing the system, which in turn allows more accurate estimation of future changes.
...
*xref:garden::technical-debt.adoc[Technical debt]* (aka. design debt) is a popular metaphor for explaining the value of standalone refactoring work to customers. When writing a refactoring ticket, consider the short-term costs and potential risks versus the longer-term benefits to be gained by implementing the proposed changes. Is there any data that can be generated to measure the impact of these changes, such as extensions to test coverage, reductions in the number of lines of code, or improvements to code complexity metrics such as Cyclomatic Complexity or NPath?

== Classifying technical debt

Not all technical debt carries the same urgency. When triaging refactoring tickets, classify each item of debt into one of three types:

1. *Blocking now.* The debt is actively preventing work — a brittle module that breaks on every unrelated change, or a dependency so entangled that a routine fix can't be made safely. This type SHOULD be minimized: prioritize it above other refactoring work, because its cost compounds with every change that touches the affected area.

2. *Will block later.* The debt does not yet obstruct work, but its trajectory makes future obstruction predictable — a data model that will not scale to an already-planned feature, or a pattern being copy-pasted across a growing number of call sites. This type SHOULD be the primary focus of standalone refactoring tickets: it is cheaper to address while the blast radius is still small.

3. *Might block later.* The debt is speculative — a design choice that could become a problem under conditions that may never materialize. This type SHOULD generally be left alone. Raising a ticket for every theoretical imperfection dilutes attention from the debt that is actually costing the team time, and much of it will never need to be paid down at all.

When multiple refactoring tickets compete for attention, prioritize using this classification: debt that is blocking now first, debt that will block later next, and debt that might block later last, if at all.
