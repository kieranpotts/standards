...
Preserve the original capitalization of a product, tool, or binary name as its owner writes it — write `Markdown`, not `markdown`; `GitHub`, not `Github` or `github` — even where that breaks the sentence-case convention in <<Headings>> or normal mid-sentence lowercasing. A proper noun keeps its own capitalization regardless of where in a sentence or heading it falls.
