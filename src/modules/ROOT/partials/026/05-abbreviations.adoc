...
Form the plural of an abbreviation by adding a lowercase "s," with no apostrophe: "three APIs," "two CDNs" — see <<Apostrophes>>. Choose "a" or "an" before an abbreviation by how it's pronounced, not how it's spelled: "an HTTP request" (pronounced "aitch"), "a URL" (pronounced "you"). Omit periods from an abbreviation written in capitals (`API`, not `A.P.I.`). Do not precede a pronounceable acronym with "the" where the acronym itself already functions as the noun: "JSON is a text format," not "the JSON is a text format" — though "the API returns JSON" is fine, since "API" there is a countable noun.
