...
* *Tuning model behavior* controls _how_ the model samples its output — the inference-time parameters that shape determinism, length, creativity, and depth of reasoning, without changing the model or its input.

* *xref:garden::context-engineering.adoc[Context engineering]* controls _what the model sees_ — the tokens placed in its working memory on each call: instructions, retrieved knowledge, tool definitions, and conversation history.

* *Harness engineering* controls _the environment the model runs in_ — the tools, guardrails, and sensors that determine what an agent can and cannot do.

* *xref:garden::loop-engineering.adoc[Loop engineering]* controls _how the whole workflow runs itself_ — automating a process end to end, so it wakes on a schedule, discovers its own work, and feeds its own output back as the next input.
