...
* *Context engineering* controls _what the model sees_ — the tokens placed in its working memory on each call: instructions, retrieved knowledge, tool definitions, and conversation history.
...
* *Loop engineering* controls _how the whole workflow runs itself_ — automating a process end to end, so it wakes on a schedule, discovers its own work, and feeds its own output back as the next input.
