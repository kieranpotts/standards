...
A single MCP client can connect to many MCP servers at once, giving it a large, composable action space. This makes MCP particularly relevant to the <<agentic-workflows,agentic workflows>> described earlier in this standard.
...
Data returned by an MCP server — web pages, file contents, API responses — is external content and a vector for indirect <<prompt-injection,prompt injection>>.
...
* Audit agent execution traces — the session transcript (see <<Auditability,auditability>>) — when tasks involve servers that fetch untrusted data.
...
* Combined with <<least-privilege-tool-access,least-privilege access>>, a poisoned or lookalike tool can still only reach what the agent's credentials and connected tools permit.
...
Remote MCP servers MUST authenticate their clients and MUST use encrypted transport. An unauthenticated MCP server MUST NOT be exposed on a public network interface — the same <<network-exposure,network-exposure discipline>> that applies to local model servers applies here too.
...
Connecting many MCP servers indiscriminately therefore degrades performance, through <<context-engineering,context rot>>, and increases cost, through <<cost-optimization,per-token billing>>.
