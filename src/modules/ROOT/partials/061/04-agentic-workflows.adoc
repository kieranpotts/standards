...
* *Portability.* Reusable context encodes rules, not knowledge, staying technology- and domain-agnostic so it runs unmodified across projects and across models (see the section on <<agent-skills,agent skills>>).
...
* *Reusability*: A well-scoped sub-agent (eg. a "Security Auditor" agent) becomes a composable unit that other workflows can invoke, not just the supervisor that first defined it (see <<composable-pipelines,Composable pipelines>>).
