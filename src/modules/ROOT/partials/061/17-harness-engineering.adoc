...
* *The checks that fire.* Validation gates bound to points in the agent's loop, discussed in <<hooks-and-deterministic-enforcement,Hooks and deterministic enforcement>> and in the section on <<guides-and-sensors,agentic workflows>>.
...
Hooks map onto both halves of the guides-and-sensors taxonomy set out in the section on <<guides-and-sensors,agentic workflows>>:

* A hook that runs _before_ an action, and can block it, is an *enforced guide*. It is a programmable extension of a permission list: where a permission rule can say only that a tool is allowed or denied, a hook can decide against the arguments and the current state of the working tree.

* A hook that runs _after_ an action is an *enforced deterministic sensor*. The formatter, the linter, the type checker, or the test suite runs because the harness ran it, not because the agent chose to.
...
A harness is not configured once. It accumulates rules in response to observed failure, and it decays as models, tools, and the codebase change around it. The same principles that govern <<reusable-context,reusable context>> apply to the whole harness:
...
* Change one thing at a time, and measure. Harness configuration is one of the components an eval suite exists to evaluate — see the section on <<Evaluation,evaluation>>. A new check, a narrowed permission, or a rewritten instruction either moves the pass rate, latency, or token cost, or it is not earning its keep.
...
Harness configuration is the least portable layer of an AI toolchain. Reusable context has converging conventions in <<the-agents-md-standard,`AGENTS.md`>> and <<agent-skills,skills>>. Tool integration has <<model-context-protocol,MCP>>. Configuration — permissions, hooks, model assignment, session behavior — has nothing equivalent, and each harness expresses it in its own format.
...
The recommendation in <<adopting-a-harness-versus-building-on-a-framework,the previous section>> is to default to a ready-made harness and to graduate to a framework only when a concrete limitation forces the move. The signals that the limit has been reached are reasonably clear: the workflow needs orchestration the harness cannot express, the agent must be embedded inside another product, or the configuration has become a set of workarounds fighting the harness's own loop.
