...
<<choosing-models,Choosing the right model>> and <<context-engineering,engineering its context>> are the two highest-leverage decisions in AI-assisted work. A third lever sits between them: the inference-time parameters that control _how_ a model samples its output.
...
That same trade raises the chance of <<hallucination>>: sampling away from the single most probable continuation is what lets a model generalize past its training data rather than merely echo it, but it is also what lets a fabricated continuation win out over a faithful one. There is no temperature setting that avoids this trade-off, only a choice of where a given task should sit on it — low for anything with a checkable correct answer, higher only where the value is in variety and the output will be verified regardless.
...
* *Effort* governs _how hard the model works_ overall. This covers not just the depth of the reasoning step, but the thoroughness of the surrounding actions: how much it explores, how many tool calls it makes, how much it verifies its own work, and how much preamble it produces. Effort is usually exposed as discrete levels (eg. low, medium, high, and higher still), rather than a continuous value.
