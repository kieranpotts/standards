...
Reusable context consumes tokens, depleting a model's attention budget and competing with the user's actual prompt. The same core principles of <<context-engineering,context engineering>> therefore apply:
...
* Prefer <<just-in-time-retrieval,just-in-time retrieval>> over always-on loading. For example, rather than embedding a full coding standard in a context bundle, instead provide a pointer to it and let the agent load it when it needs it.
...
`AGENTS.md` covers the static half of reusable context — what every session needs to know up front. The dynamic half, loaded on demand as a task requires it, is covered in the section on <<agent-skills,agent skills>>.
