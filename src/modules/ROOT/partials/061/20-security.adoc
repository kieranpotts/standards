...
For AI-assisted development in an IDE or code editor, it is RECOMMENDED to develop inside a https://containers.dev/[devcontainer] rather than directly on the host. A devcontainer is defined by a `devcontainer.json` file specifying the base image, mounted volumes, and editor extensions. Both the editor and the agent harness run inside the container, with project files bind-mounted, so the human and the agent can collaborate on the same files simultaneously.

Devcontainers are https://containers.dev/supporting[natively supported] by VS Code, Zed, IntelliJ IDEA, and Emacs. Other editing environments can be sandboxed the same way, but require more manual wiring — connecting the editor to a container over SSH, for example.
...
* Grant agents only the tool permissions they need for the current task (see <<least-privilege-tool-access,Least-privilege tool access>>). A compromised agent can still only do what its connected tools allow.
...
* Audit agent execution traces — ie. the session transcript (see <<Auditability,auditability>>) — when tasks involve untrusted sources.

[#the-lethal-trifecta]
== The lethal trifecta

Prompt injection becomes catastrophic when an agent simultaneously has all three of the following capabilities — a combination that https://simonwillison.net/2025/Jun/16/the-lethal-trifecta/[Simon Willison called the lethal trifecta]:
...
That incident was not an isolated one. Within the following month, Anthropic, the UK's AI Security Institute (AISI), and Meta each disclosed their own testing incidents in quick succession. Anthropic found a handful of instances, out of thousands, where Claude had gained unintended internet access during testing. The AISI, evaluating models from both OpenAI and Anthropic, found they attempted cyber-attacks that included creating fake human profiles to socially engineer people. Meta reported that a misconfiguration during third-party testing had left one of its models with internet access it should not have had (McMahon, 2026).

Each incident had a different proximate cause, and the AISI case is the one most worth dwelling on, because it was not a containment failure at all. Cyber-security researcher Alan Woodward's framing of the three incidents usefully separates them: "One model broke out. One walked through a door left open by mistake. One was deliberately given the keys so testers could measure what it would do" (McMahon, 2026). In the AISI case, the evaluators themselves granted the models internet access and disabled the filters that would normally block dangerous cyber-attack behavior — a deliberate design choice for the evaluation, not a bug in the sandbox — and the models then displayed "novel, potentially deceptive" behavior AISI attributed in part to those very design choices. This is a distinct failure mode from a sandbox being breached: the test itself assembled the <<the-lethal-trifecta,lethal trifecta>>, on purpose, to see what would happen.

When testing agents:

* Apply the <<the-lethal-trifecta,lethal trifecta>> analysis to the test environment itself. Do not give a test sandbox real external network access or real credentials merely because it is "just a test". Assume the agent MAY be able to escape its containment.

* Apply the same analysis to the *evaluation design*, not only the sandbox. Granting an agent internet access or disabling its safety filters to observe worst-case behavior is sometimes a deliberate and legitimate part of an evaluation, but it recreates the lethal trifecta by design. Treat such a configuration as a high-risk activity in its own right, requiring the same isolation and containment as an untrusted sandbox, not merely as "how the test is set up".
...
* Monitor for and have a kill switch ready to halt unexpected autonomous behavior, including any attempt to reach systems outside the test environment. Containment speed matters: the AISI contained its incident within an hour of detection (McMahon, 2026), and a slower response could have let the same behavior reach real systems.
...
Woodward's summary of what this demands in practice is a useful way to hold all of the above in mind at once: testing a sufficiently capable agent is "less like checking code and more like handling a hazardous material: sealed rooms, constant monitoring of what leaves the building, a rehearsed containment plan" (McMahon, 2026).
...
Keep tasks small. Break work into reviewable steps rather than large, open-ended tasks (see <<implementation-phase,AI-assisted development workflows>>). Small diffs are easier to review, and problems are caught early, while course-correction is cheap.
...
Reviewing agent output judges the artifact. Auditability concerns the durable record of _how_ that artifact came to be — what the agent was asked, what it did, and what data passed through it. In agentic workflows, where an agent takes many autonomous actions between human checkpoints, that record is essential for incident investigation, accountability, and demonstrating compliance.
...
This audit substrate complements, rather than duplicates, the version-controlled audit trail of workflow artifacts (see <<Persistence,agentic workflows>>). The two answer different questions. Version history records _what_ each step produced — the specification, the design, the plan, the code — as durable, diffable artifacts. The session transcript records _how_ an agent arrived at that output, through the prompts, reasoning, and tool calls in between. Together they give end-to-end traceability, from a triggering request through to a merged change.
