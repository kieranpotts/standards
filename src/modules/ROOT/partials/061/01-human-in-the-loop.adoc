...
Narrow forms of autonomous decision-making are not new — expert systems, planning algorithms, and search-based engines have made decisions and produced novel-looking output within hand-built domains for decades. What is new with LLM-based AI is the generality: the same tool now appears to decide and originate ideas across open-ended domains, in natural language, without a domain-specific system built around it first. That apparent generality is what makes it tempting to treat AI output as decided rather than merely produced, and to defer to it accordingly.
...
The third is xref:garden::accountability.adoc[*accountability*]. When a system harms someone, a human is answerable for it — legally, professionally, and ethically. A model cannot hold that responsibility, so a human must remain in the loop by definition.

== Fallibility awareness is not a substitute

Model providers are making genuine progress on training models to recognize their own fallibility — to express calibrated uncertainty, ask clarifying questions rather than guess, and admit mistakes instead of defending them. Being able to say "I don't know" is a mark of wisdom in humans as much as in machines, and a model that has learned this is a safer collaborator than one that hasn't.

That progress is welcome, but it is not sufficient on its own. No degree of self-awareness engineered into a model can substitute for keeping a human in the loop. AI capabilities are moving too fast, and in directions too unpredictable, for any team to anticipate every failure mode far enough in advance to design it out of the model itself. Self-doubt trained into a model is a property of the model at the time it shipped; the risks a team needs guarding against are the ones that show up after that, in production, under conditions training didn't anticipate. Human oversight is what catches those.

[#ai-augmented-software-development]
== AI-augmented software development

The objective, therefore, MUST NOT be to design autonomous replacements for human decision-making. Instead, the goal should be *AI-augmented software development*: humans directing AI tools, and remaining firmly in control of the environments in which those tools operate — focusing the tools on the work best placed to augment our own expertise, knowledge, and judgment. This is a posture about where the human stays; the case for _why_ this augmentation pays off, and what it costs, is made under <<benefits-and-costs,Benefits and costs>>.
...
There MUST always be a human-in-the-loop when using AI tools for software development. Only the _degree_ of human involvement may change from one project to the next.
...
A throwaway prototype, an internal script, or an experiment can be driven with a light touch and reviewed loosely. <<vibe-coding,Vibe coding>> is well suited to anything that doesn't need to be production-grade, and to research and development work with no clear end goal.
