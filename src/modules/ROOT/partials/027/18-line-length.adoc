...
In CommonMark, a single newline inside a paragraph is a soft break, and it renders as a space. A paragraph split across several source lines therefore renders identically to the same paragraph written on one line. How prose is wrapped in the source is a decision about diffs, editing, and review, not about rendering. The same is true of most other lightweight markup languages.

The one exception is a line break that falls inside an inline construct — a bold or italic span, an inline code span, a link. Depending on the construct, the break can separate the markup from the text it applies to, so it may silently fail to render, or fail loudly by rendering the raw syntax as literal text. A wrapped line that happens to begin with a list marker, `#`, or `>` may also be parsed as the start of a new block.
...
* *Benefits.* It is the simplest approach. There is no reflow, and no wrap points to break an inline construct. It matches what most WYSIWYG-adjacent tools, such as note apps and Markdown exporters, produce.
...
* A repository SHOULD choose one approach for its Markdown prose and record the choice where contributors will find it, such as its contributing guide or `AGENTS.md`.
...
* A link MUST NOT be broken across lines (see <<links,Links>>), so a line that contains a long link may run past the wrap column.
...
* The cells of a table row MUST each sit on a single source line, because the pipe table syntax requires it.
...
Wrapping applies to prose only. Fenced and indented code blocks keep the line structure their content needs, and code samples follow the line-length rules of the relevant language standard.
