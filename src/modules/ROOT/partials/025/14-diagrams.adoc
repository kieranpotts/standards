...
A diagram is warranted when a structure or flow is easier to understand visually than as prose. Good use cases for diagrams include the documentation of component boundaries, data flows, state machines, and sequence of calls across systems.

A diagram SHOULD NOT be the only place a piece of information is recorded. Diagrams SHOULD be accompanies with prose that conveys the same information the diagram conveys. This is necessary to enable keyword search, and it also supports use of assistive technologies to consume the documentation.
