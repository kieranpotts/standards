...
Prefer writing functions that either perform an action (a "command") or compute and return a value (a "query"), but not both. A function that mutates state _and_ hands back a result is harder to reuse and to reason about, because callers cannot invoke it for one purpose without triggering the other.
