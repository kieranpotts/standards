...
The exception is a language-specific tool – a linter or formatter for a particular language, for example – where the user can safely be assumed to already have that language's interpreter or runtime installed. Such a tool MAY instead be distributed through the language's own package manager (eg. `npm`, `pip`), since that is where its users already look for tools in that ecosystem.
