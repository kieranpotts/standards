...
*Shell.* The command-line interpreter a user interacts with directly – Bash, Zsh, `cmd.exe`, PowerShell, etc. Its primary purpose is to read commands and start other programs. A CLI program is normally invoked from a shell, but is not itself a shell.
