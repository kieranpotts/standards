...
Terraform includes a built-in test framework, invoked with the https://developer.hashicorp.com/terraform/cli/commands/test[`terraform test`] command. It runs configuration files in the `tests/` directory against the module in the current working directory, executing real Terraform operations (plan, apply, destroy) in isolated, temporary state. This complements the static analysis described in <<testing,Testing>> — `terraform test` verifies the behavior of a configuration, not just its syntax.
...
* `plan` — Runs `terraform plan` and makes the planned values available for assertions, without applying changes. Use `plan` for fast, side-effect-free checks of what _would_ happen.
...
For testing the behavior of deployed infrastructure — that a web server responds, that a database is reachable — continue to use the dynamic testing tools described in <<testing,Testing>>. `terraform test` validates the configuration; it does not validate the running system.
