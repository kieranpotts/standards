...
A coverage report tells you which lines are untested. It does not tell you which of them are worth testing. Some code does not warrant a test of its own, and some code is actively worth _not_ testing.

Tests MUST NOT be written to verify behavior supplied wholly by a framework, a library, or the language runtime. That code is already tested by its maintainers, who know it far better than its consumers do, and re-testing it buys nothing. Such a test cannot fail for any reason you are able to act on, it adds runtime to every subsequent run of the suite, and it usually has to reach into the dependency's internals to assert anything at all — coupling the suite to implementation details that are not yours and that may change on any upgrade.

The discriminator is to ask whether the assertion would still hold if your own code were deleted. If it would, you are testing the framework. Asserting that an ORM writes a row when its own `save` method is called, that a state-management hook re-renders the view when its setter runs, or that a validation library rejects a malformed email address against one of its own built-in rules — each of these is a test of somebody else's code, and belongs in somebody else's test suite.

What remains yours, and MUST still be covered, is everything you contributed to the arrangement:

* *Your configuration of the dependency.* Custom validation rules, schema and mapping definitions, routing tables, access policies, and any setting moved off its default are your logic expressed in the dependency's vocabulary. Test the outcomes they produce, not the mechanism that applies them.

* *Your integration with the dependency.* That your code invokes the right operation, with the right arguments, at the right point in a workflow is your behavior and not the dependency's. This is the proper subject of an integration test — see <<integration-tests,Integration tests>>.

* *Your own logic.* Everything the framework did not supply, including the branches and error paths around the call into it.

There is one exception. Where you depend on behavior you have specific reason to distrust — a known upstream defect you are working around, or an undocumented behavior the design leans on — a test that pins that behavior earns its place. It is not a test of the dependency; it is a regression test of an assumption, and its value is that it fails when an upgrade invalidates the assumption rather than leaving you to discover it in production. Such a test SHOULD state the assumption and cite the upstream issue in a comment, so that a later reader can retire it once it is no longer needed. See <<regression-testing,Regression testing>>.

=== Small components need fewer tests

Where the boundary between framework behavior and your own behavior falls is itself a design decision. A small component leaves most of the heavy lifting to the framework and contributes little logic of its own, so there is little to test, and what there is can be reached directly. A large component interleaves its own logic with framework behavior until the two cannot be exercised separately, and the tests that result are both more numerous and unavoidably tests of the framework as well.

A component that appears to demand a large number of tests SHOULD therefore be read as a design signal before it is read as a testing problem. Splitting it usually removes more test cases than it adds. This is the same observation as the one quoted in <<test-driven-development,Test-driven development>>: a testability problem points to an underlying design problem. See xref:007.adoc[TS-7] for component sizing and separation of concerns.
