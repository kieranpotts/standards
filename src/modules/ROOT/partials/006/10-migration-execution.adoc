...
xref:005.adoc[TS-5] covers when service extraction is premature. This section covers how to execute an extraction once a component's interface has stabilized and the prerequisites in <<microservices-at-scale,Microservices at scale>> are in place. It complements xref:045.adoc[TS-45], which covers the mechanics of moving the underlying data — extracting a service almost always requires migrating the data it owns, and that data migration SHOULD follow TS-45's expand-and-contract discipline in full.
...
1. Introduce a routing layer (a proxy, gateway, or feature flag) in front of the monolith, capable of directing individual requests to either the monolith or a new service.
...
4. Repeat for the next piece, until the monolith serves nothing that has not already been extracted, at which point it can be retired.
...
A migration plan SHOULD sequence extractions explicitly, in writing, rather than extracting opportunistically — see xref:003.adoc[TS-3] for how to capture that plan.
