...
4. *Eventual consistency.* Given no new writes, all replicas will *eventually* converge on the same value, with no bound on how long convergence takes and no guarantee about the order in which intermediate states are observed. The weakest and cheapest guarantee, and the default for AP systems.
