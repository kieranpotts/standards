...
A single-database transaction gives atomicity for free: either every change commits, or none does. Once a business operation spans more than one service — each with its own database, per xref:005.adoc[TS-5]'s guidance that each service owns its own state — that guarantee disappears, and the system MUST decide explicitly how it recovers when one step succeeds and a later step fails.
...
The *transactional outbox* pattern resolves it. The service writes the event to an "outbox" table in the same local transaction as the business data it describes, so the two either both commit or both roll back — they cannot disagree. A separate process (a poller, or change-data-capture reading the database's replication log) then reads the outbox table and publishes the event to the message broker, retrying until it succeeds, and marks the row as published only on confirmed delivery. This gives at-least-once delivery of the event (see <<idempotency-and-delivery-semantics,Idempotency and delivery semantics>>) with no possibility of a write persisting silently without ever being announced, and without requiring a distributed transaction between the database and the message broker.
