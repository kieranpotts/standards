...
* For frequently-accessed data, use *xref:garden::caching.adoc[caching]* layers to reduce the number of requests to the underlying storage system, and implement *compression* on both data-at-rest and data-in-transit wherever possible.
...
The first of these strategies is a quality trade-off, not merely a configuration choice. Capping auto-scaling lowers the availability the system can offer under surge, in exchange for a bounded bill. Where an availability threshold has been specified, the cap MUST be set consistently with it, and a cap that cannot be reconciled with the specified threshold is a requirements problem to escalate rather than a value to tune. See xref:001.adoc[TS-1] for specifying and costing availability requirements, and xref:002.adoc[TS-2] for resolving the underlying conflict between availability and cost.
