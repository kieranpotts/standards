...
Much cloud spend is committed long before an invoice arrives, at the point somebody sets an availability, latency, or data-retention threshold. Those thresholds are quality requirements, and the standard requires each to be justified by weighing the value of meeting it against the cost of meeting it — see xref:001.adoc[TS-1]. A cost problem that traces back to an unjustified threshold is not solved by tuning the infrastructure.

For guidance on cloud platform engineering — including multi-account strategies and environment management — see xref:049.adoc[TS-49].
