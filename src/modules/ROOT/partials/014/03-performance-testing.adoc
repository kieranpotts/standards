...
* xref:garden::throughput.adoc[*Throughput*] — the number of transactions or requests the system can process per unit of time.

* xref:garden::resource-utilization.adoc[*Resource utilization*] — CPU, memory, disk I/O, and network bandwidth consumed during operation. High utilization under normal load may indicate capacity risks.
...
A performance test reports a symptom — a slow endpoint, a p99 spike, a throughput ceiling — not a cause. Finding the cause routinely requires reasoning about layers below the one where the symptom was observed: the runtime or interpreter executing the application code, the code the compiler or JIT actually generated, the operating system's scheduling and I/O behavior, and the hardware's cache and memory characteristics. A slow Python function may be slow because of an algorithm, because of CPython's object model, or because of cache misses in the C extension it calls into; the test result alone does not say which.

This is a distinct skill from writing the test itself, and it does not develop automatically from running tests. Engineers build it by habitually asking, when a result is surprising, what the next layer down would have to be doing to produce it — then confirming with a profiler, a debugger, or the layer's own instrumentation rather than guessing.

* *Profile before hypothesizing.* A flame graph or sampling profiler narrows the search to the actual hot path before any layer-specific investigation starts. Reasoning about the wrong layer wastes as much time as not reasoning about layers at all.

* *Keep one mental model per layer, not one model of the whole stack.* Treat the interpreter's dispatch loop, the OS scheduler, and the CPU cache hierarchy as separate, composable explanations. A regression is usually explained by one layer behaving unexpectedly, not by the interaction of all of them at once.

* *Reproduce the symptom at the smallest layer that shows it.* A benchmark that isolates the interpreter, the query planner, or the syscall in question, stripped of the surrounding application, turns a vague production slowdown into a reproducible, layer-scoped test case.

* *Treat this as an engineering practice to grow deliberately, not a prerequisite to hire for.* Teams build this capability over time by pairing on performance investigations and writing up root causes — not by filtering for it up front, which needlessly narrows the hiring pool.
