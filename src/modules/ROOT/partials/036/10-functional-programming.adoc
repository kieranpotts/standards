...
At the most fundamental level, a functional program is one that makes extensive use of functions to create a coding style that is more declarative than imperative. While imperative code is very _explicit_ about how it works, a declarative programming style is more _implicit_ about this.
...
Functional programming, which is a declarative programming paradigm, abstracts logic behind functions. The function names describe _what_ the logic does, hiding details of the implementation (_how_ it works). But for a program to be described as having been written in a fully functional style – rather than a more general declarative style – additional design constraints need to be evident.
...
Higher-order functions are functions that take another function as an argument, or that return a new function, or both. So, instead of operating on primitive values such as integers or booleans, or on data structures, these functions go _higher_ and operate on other functions.
...
ECMAScript's own standard library has many, _many_ higher-order functions. Examples include `Array.prototype.map` and `setTimeout`.
...
Currying is another functional programming pattern. This technique is named after https://en.wikipedia.org/wiki/Haskell_Curry[Haskell Curry], a mathematician who was hugely influential in the development of early computer programming languages.
...
But now you have more flexibility. You could apply only _some_ of the arguments, and then apply the remainder at a later time. This is known as partial application.
...
https://lodash.info/doc/curry[Lodash's `curry` function] can be used to turn any regular function into a curried one. However, it is generally better practice to design functions to support currying by default. This means designing function signatures with the following constraints:
...
Out-of-the-box, most ECMAScript, web and Node APIs do not support currying. Consider for example ECMAScript's implementation of `map`. It cannot be curried because https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/map[it is a method of an `Array` object] rather than a standalone function. Even https://lodash.info/doc/map[Lodash's `map` function] cannot be curried because it puts the data object as the first argument.

Unfortunately this means a lot of extra coding is often needed before function composition can be done in JavaScript. Native APIs, and even most userland libraries, don't make this easy. But happily there are some great utility libraries dedicated to solving the problem of doing functional programming in JavaScript. https://github.com/lodash/lodash/wiki/FP-Guide[Lodash's FP variant] and the excellent https://ramdajs.com/docs/#map[Ramda] and https://www.7urtle.com/[7urtle] packages all provide extensive libraries of curried functions, including for `map`.
...
[NOTE]
======
Currying supports what is known as _tacit programming_ or _point-free style_ or _point-free composition_. These terms refer to a programming paradigm where function definitions do not identify the arguments (or "points") they operate on. Instead, function definitions merely compose other functions, as in the second example below:
...
Because a pure function does not depend on or alter any external state, it should always return the same output given the same input. The following function is _not_ a pure function. That's because if you called this function multiple times, even with the same argument, you will get a different result each time.
...
This is why JavaScript is not a pure functional programming language. In _bona fide_ functional languages you cannot mutate data – period! Happily, immutable methods are being incrementally added to the ECMAScript specification. For example, https://github.com/tc39/proposal-change-array-by-copy[ES 2023 standardized] `Array.prototype` methods like `toReversed` and `with` that make it easier to change arrays in an immutable fashion. But, in general, to enforce immutability in JavaScript applications, programmers need to take responsibility for this themselves.
...
So in practical terms, specialist userland libraries are needed that offer real immutable data structures. Examples of such libraries include https://immutable-js.com/[Immutable.js], https://immerjs.github.io/immer/[Immer], and https://swannodette.github.io/mori/[Mori]. These libraries offer alternative implementations of native JavaScript data structures such as https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Map[Maps] and https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Set[Sets].
...
Using Immutable's https://immutable-js.com/docs/v4.3.6/Map/[Map] implementation instead gives you the same data structure but in an immutable form:
...
These techniques are particularly beneficial in asynchronous and event-driven programs. Reactive libraries, including https://react.dev/[React] itself, are often designed around FRP principles.
======
