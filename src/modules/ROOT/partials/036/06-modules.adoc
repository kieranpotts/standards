...
Although all major runtime environments, including browsers and server-side runtimes, now support ECMAScript modules, sometimes it may be necessary to transform ESM code before it can be consumed by other tools or environments. A static transcompiler like https://babeljs.io/[Babel] or a runtime transcompiler like https://www.npmjs.com/package/esm[esm] can be used for this purpose. To target web browsers, further "bundling" may be beneficial, to reduce distributable builds. An ESM-compatible bundler like https://rollupjs.org/guide/en/[Rollup] or https://webpack.js.org/[Webpack 2] may be used for this purpose.

== Background

For many years in its early history, JavaScript did not have a native module system. https://www.ecma-international.org/[Ecma International] standardized ECMAScript Modules – aka ES Modules, ESM, or Harmony Modules – in the 6th edition of ECMAScript, finalized in 2015.
...
Alternatively, transpilation or bundling tools like Webpack can be https://webpack.js.org/configuration/resolve/#resolvealias[configured] to define path aliases:
...
In TypeScript, this is easier still. TypeScript has a https://www.typescriptlang.org/docs/handbook/module-resolution.html[path mapping] option that maps arbitrary filesystem paths – anything that does not start with `/` or `.` – to physical paths. Path maps are defined in `tsconfig` and resolved automatically by the compiler:
...
At the time of Node's first release in 2009, JavaScript did not have a native module system, so Node adopted https://wiki.commonjs.org/wiki/Modules[CommonJS Modules] (CJS) – a community-derived module system implemented as extensions to JavaScript's standard API: the `require()` function and the `module.exports` object (and its alias `exports`).
...
It is also possible to import modern ES modules into legacy CJS scripts. Because ES modules are loaded asynchronously and Node's `require()` is synchronous, a different API is needed: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/import#dynamic_imports[dynamic imports], also known as `import()` expressions:
