...
* *`Promise.all`.* Resolves with an array of results once _all_ input promises fulfill, or rejects as soon as any one rejects.
...
If you want to handle data as it becomes available when individual promises in a collection resolve – rather than waiting for all of them – use _async iterators_. The `for await...of` construct is the easiest pattern for iterating through the results of a collection of promises; `forEach` cannot be used (you cannot `await` it).
