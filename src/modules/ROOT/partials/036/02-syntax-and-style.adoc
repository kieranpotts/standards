...
The same principle applies to chains of method calls. A dense chain saves a few lines but forces the reader to mentally unwind each step before they can follow what the code does. Naming each intermediate result documents the transformation as they read. See also xref:007.adoc[TS-7].
...
*Naming.* Prefer explicit names over implicit ones: `sleepForSeconds(t)` is clearer than `sleep(t)`, because the unit the parameter expects is evident from the function name alone. (See <<naming-conventions,Naming conventions>>.)
...
Protocols that describe _what_ something SHOULD read as nouns, e.g. `Collection`. Protocols that describe a _capability_ SHOULD be named using suffixes like "able", "ible" or "ing", e.g. `Equitable`, `ProgressReporting`.
...
`const` makes the *binding* immutable, not the value. For primitives this means the value cannot change; for objects, the binding cannot be reassigned but the object's properties can still be mutated. To make an object truly immutable, combine `const` with `Object.freeze()` (see <<objects-and-classes,Objects and classes>>).
...
Comments are annotations in source code that are ignored by parsers but let developers leave notes for each other. Start by making programs as self-documenting as possible – meaningful names, clear structure – but well-written code documents only _what_ a program does, not _why_ the developer chose a given approach. Comments transfer that remaining knowledge to other developers.
...
In the JavaScript ecosystem, variations on the syntax supported by https://jsdoc.app/[JSDoc] are widely used to embed structured API information, including type information, into source code comments. These tools have now been largely supplanted by TypeScript: the TypeScript compiler supports an https://www.typescriptlang.org/docs/handbook/jsdoc-supported-types.html[extended subset of JSDoc], and the emerging https://tsdoc.org/[TSDoc] specification further extends TypeScript-flavoured JSDoc for a wider range of tools.
