...
Functions encapsulate much of a program's logic, so how they are named and structured goes a long way toward self-documenting code (see <<naming-conventions,Naming conventions>>). The use of functions as higher-order functions is covered in <<functional-programming,Functional programming>>.
...
A closure is formed when an inner function retains access to an outer function's variables after the outer function has returned. Closures are useful for encapsulating private state, but they can also hide circular references that cause memory leaks (see <<quality-assurance,Quality assurance>>).
...
It is rarely used, but you can use https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/label[labeled statements] to identify loops. This is useful in nested loops, as it allows you to specify which loop to `continue` or `break` out from.
