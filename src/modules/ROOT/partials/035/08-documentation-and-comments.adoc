...
Every public module, class, function, and method MUST have a docstring, per https://peps.python.org/pep-0257/[PEP 257]. A private (single-underscore) function SHOULD have one if its behavior is not obvious from its name and signature alone.
...
This standard adopts https://google.github.io/styleguide/pyguide.html#38-comments-and-docstrings[Google-style] docstring sections — `Args:`, `Returns:`, `Raises:` — over the alternative reStructuredText (`:param:`, Sphinx-native) or NumPy styles, because they read cleanly as plain text even before any documentation generator processes them.
...
`# TODO(username): description` SHOULD be used to mark unfinished work, following the convention recommended by https://google.github.io/styleguide/pyguide.html#312-todo-comments[Google's Python Style Guide], with the username identifying who to ask for context — not necessarily who will do the work.
