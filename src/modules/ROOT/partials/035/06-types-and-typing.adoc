...
* `Optional[X]` / `X | None` MUST be used explicitly — no implicit `None` default
...
Every public function, method, and module-level variable MUST carry type hints for its parameters and return value, per https://peps.python.org/pep-0484/[PEP 484] and https://peps.python.org/pep-0526/[PEP 526]. Private (single-underscore) helpers SHOULD also be annotated, but the requirement is stricter for public interfaces because they are the contract other code depends on.
...
The builtin generic syntax (`list[Item]`, `dict[str, int]`) MUST be used instead of the deprecated `typing.List`, `typing.Dict`, etc., which are retained only for backward compatibility with Python versions predating https://peps.python.org/pep-0585/[PEP 585] (3.9). A project supporting an older Python version MUST use the `typing` equivalents, or `from __future__ import annotations` to defer evaluation and use the builtin syntax anyway — see <<deferred-evaluation-of-annotations,Deferred evaluation of annotations>>.

A value that may be absent MUST be annotated as `X | None` (Python 3.10+, per https://peps.python.org/pep-0604/[PEP 604]) or the equivalent `Optional[X]` on earlier versions — never left as a bare `X` with an implicit `None` default silently tolerated by the caller. A function returning `X | None` signals to every caller that the `None` case MUST be handled.
...
A static type checker — https://mypy-lang.org/[mypy] or https://microsoft.github.io/pyright/[pyright] — SHOULD run in CI against the whole codebase, not merely on a subset of files, and its failures SHOULD block the build. Where a codebase is being migrated to typed code incrementally, the checker MAY be configured to only enforce strict checking on already-annotated modules, with a plan to widen that scope over time, rather than left permanently partial.
