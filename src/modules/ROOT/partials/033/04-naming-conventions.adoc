...
A *constant* is defined here as a static final field whose contents are deeply immutable and whose methods have no detectable side effects. Merely _intending_ to never mutate the object is not sufficient. Examples of valid constants include primitives, strings, immutable value classes, and anything set to `null`.
...
* A name in the form used for classes, followed by the capital letter `T`, eg. `RequestT`, `FooBarT`.
