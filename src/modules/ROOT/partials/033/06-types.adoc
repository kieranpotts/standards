...
The opening `"""` MUST be on its own line, and the closing `"""` MUST also be on its own line, indented to match the indentation of the text block's content. Each line of text within the block MUST be indented at least as much as the delimiters.

Unlike an ordinary string literal, a text block's contents MAY exceed the standard's column limit, since reflowing the text would change the string's value.
