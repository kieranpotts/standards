...
The opening `"""` is always followed by a line break, as the language requires. The closing `"""` MUST be on its own line, indented to match the indentation of the text block's content. Each line of text within the block MUST be indented at least as much as the closing delimiter, since the least-indented line determines how much indentation is stripped from every line.
...
A method whose return type is not `Optional<X>` MUST NOT return `null` under any circumstance. Where a meaningful value genuinely cannot be produced, throw an exception instead of returning `null` from a non-`Optional` return type. The possibility of an absent return value MUST always be communicated using type `Optional<X>`.
...
None of these libraries is bulletproof. Each works differently, and none can guarantee null-safety the way a language with built-in non-nullable types can. But where `Optional` does not fit, an annotation library is the best mechanism Java offers.
