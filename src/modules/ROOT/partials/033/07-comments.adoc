...
documentation is inherited. Javadoc MAY also be skipped for a member that is
"simple" and "obvious" — a trivial getter or setter, or a no-arg constructor
that is not itself overridden — provided there is truly nothing else
worthwhile to say about it.

A private member SHOULD carry Javadoc where it is complex, or where it is
called from multiple places. Where a private method is called from only one
place, its documentation MAY instead live in the calling method's Javadoc.
