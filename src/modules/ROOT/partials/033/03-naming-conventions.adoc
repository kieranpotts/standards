...
The single underscore, `_`, MAY be used as an unnamed variable or parameter wherever Java's unnamed-variable syntax applies — for a local variable, pattern-match binding, or lambda parameter whose value is never read. It signals to the reader that the value is deliberately unused.
...
Records SHOULD follow the same naming conventions as classes: UpperCamelCase, typically a singular noun or noun phrase describing the data they hold, eg. `Point`, `OrderLine`.
