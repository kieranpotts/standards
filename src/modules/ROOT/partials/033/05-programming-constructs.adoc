...
Wildcard imports, static or otherwise, SHOULD NOT be used. Module imports (`import module java.base;`, JDK 25 or later) MUST NOT be used either. A module import brings in every public type of every package the module exports, so it has the same drawbacks as a wildcard import, at a larger scale: a reader cannot tell where a name comes from, and a new type added to any of those packages can make an existing simple name ambiguous.
...
There SHOULD be exactly one blank line between the two groups to separate them. Within each group, imports SHOULD be sorted in ASCII order of the imported names. This is not quite the same as sorting the import lines, because `.` sorts before `;`: `com.example.Foo` sorts before `com.example.Foo.Bar` by name, but `import com.example.Foo.Bar;` sorts before `import com.example.Foo;` as a line. There SHOULD NOT be any other blank lines between import statements.
...
[source,java]
----
/* ✗ The type is not evident from the initializer, and the name adds nothing. */
var result = service.fetch(request);

/* ✓ The type is stated explicitly. */
CustomerPage result = service.fetch(request);

/* ✓ The name carries what the type no longer shows. */
var customers = service.fetch(request);
----

In each of the following, the initializer makes the type obvious, or the `var` names an intermediate step.
