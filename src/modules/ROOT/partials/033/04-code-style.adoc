...
Other code MAY exceed the column limits, but only where line-wrapping it would reduce its readability (eg. command lines written in comments) or where keeping within the limits is simply not possible (eg. long URLs in comments). A very long identifier, on the rare occasions one is called for, is another such case.

A character, for the purpose of the column limits, is one Unicode code point, whatever its display width. A line containing fullwidth characters, which display at twice the width of ASCII, MAY be wrapped earlier than the limits strictly require.
...
Continuation lines SHOULD be indented by an additional *four spaces* — double the normal indentation level, so that they stand out from the nested block that follows. Where there are several continuation lines, two of them SHOULD share an indentation level only if they begin with syntactically parallel elements, such as successive arguments or successive calls in a chain. A continuation nested inside another MAY be indented further.

Where a chain of method calls is wrapped, each call SHOULD start its own line, beginning with the dot. Breaking a chain wherever the line happens to fill hides its structure, and leaves no obvious place for the next call to go.
...
The same applies to the parameter list of a method or constructor declaration that does not fit on one line. Rather than filling each line with as many parameters as fit, break after the opening parenthesis and put each parameter on its own continuation line. Every parameter is then equally easy to find, and adding or removing one changes a single line.
