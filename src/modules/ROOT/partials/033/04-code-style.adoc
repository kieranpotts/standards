...
An empty *statement* — a stray `;` with no effect — SHOULD be avoided. This is a different, and always erroneous, construct from an empty *block*: an empty block (`{}`) is sometimes intentional, per the guidance above, but a bare `;` is never intentional, and is usually a typo — for example, an accidental `;` immediately after an `if` condition, which silently turns the following block into an unconditional statement.
