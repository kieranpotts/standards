...
The modular monolith pattern, described in the section on <<vertical-slices,vertical slices>>, provides a natural stepping stone. Modules can be designed with clear boundaries and indirect communication from the outset, then extracted into independent services incrementally once their interfaces have stabilized.
...
This is the essence of a https://www.reactivemanifesto.org/[reactive system] — a system that mutates domain models in response to events or messages. It is a simple and powerful model, and it is how serious infrastructure (including relational databases) works internally.
...
* xref:garden::durability.adoc[*Durability*]: The messaging infrastructure must be highly available. If it goes down, services cannot process events, and the system stalls.
...
1. *State the antipattern explicitly.* Name what rule is being broken and why the clean alternative was rejected — too expensive, too slow to deliver, or blocked by a constraint outside the team's control.

2. *Bound the risk.* Identify what could go wrong as a direct result of the coupling or shortcut, and what would have to be true for it to cause an incident.

3. *Record the decision.* Capture the reasoning somewhere durable — an RFC, an architecture decision record, or equivalent — so a future engineer encountering the antipattern understands it was chosen, not missed.

4. *Revisit periodically.* An accepted antipattern is a liability the team carries deliberately, not a permanent exemption. Revisit the decision as the system's scale or requirements change, and remove the antipattern once a clean alternative becomes affordable.
