...
In client-server systems such as web applications, the client-side application may specialize in only the user interface, while the server-side application specializes in data processing, business logic, and state persistence. GUI applications tend to require a specialist architecture, which is outside of the scope of this technical standard. See xref:018.adoc[TS-18] for guidance on designing client-side web applications.

Related technical standards include xref:002.adoc[TS-2], which describes general software architectural design best practices and standards; and xref:007.adoc[TS-7], which covers concerns such as code formatting and low-level design patterns.

See also xref:046.adoc[TS-46], which is an important consideration in application architecture.
