...
One of the biggest challenges in delivering software updates to users incrementally through continuous integration and continuous deployment is that _release_ is required to be decoupled from _deployment_. Changes in code and configuration are continuously integrated and deployed to production systems, but the activation of the behaviors that those changes enable happens separately.
