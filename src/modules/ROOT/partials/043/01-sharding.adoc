...
Finally, if sharding is combined with replication and denormalization techniques, you will need to plan for how _eventual_ data consistency will be achieved across shards. See <<transactions-and-consistency,Transactions and consistency>> for the broader treatment of consistency models, isolation levels, and the ACID/BASE trade-off that this eventual-consistency planning sits within.
