...
`<button>` MUST always carry an explicit `type` — `type="submit"` for a button that submits its enclosing `<form>`, or `type="button"` for any other push-button behavior handled by JavaScript. A `<button>` with no `type` attribute defaults to `type="submit"`, which is a common source of forms being submitted accidentally by a button that was only meant to trigger some other in-page behavior.
