...
This technical standard covers document structure, the document head, tables, hyperlinks, forms and buttons, images, audio/video and embedded content, scripting conventions, metadata schemas, internationalization, and accessibility.

SVG and other adjacent technologies — any media that is commonly embedded in HTML — are also the subject of this technical standard.

For CSS, see xref:040.adoc[TS-40]. For the web platform's scripting APIs, see xref:037.adoc[TS-37]. For broader web GUI concerns, including performance optimization, see xref:018.adoc[TS-18].
