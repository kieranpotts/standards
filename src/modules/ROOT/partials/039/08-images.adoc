...
`<img>` MUST always carry `src`, `width`, `height`, and `alt`. `ismap` and `usemap` MUST NOT be used — see <<image-maps,Image maps>> for why that mechanism is obsolete.
