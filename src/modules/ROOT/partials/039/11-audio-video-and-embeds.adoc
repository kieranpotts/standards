...
* MUST use `id`, not `name`, for referencing the iframe from the parent document's own scripts or styles, consistent with <<coding-style,Coding style>>; `name` remains relevant only where the iframe is itself the target of a form submission or a link's `target` attribute.
