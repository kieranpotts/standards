...
The difference between the two is that the classic shebang bets everything on
one path, while the env shebang looks for the Bash binary in more places, and
thus the script has a greater chance of being successfully run across different
environments. The trade-off is ever-so-slightly slower startup time, as the PATH
environment variable needs to be searched to find the Bash binary.
...
Again, the second env-style shebang will have wider compatibility and is
