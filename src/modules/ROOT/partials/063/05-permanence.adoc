...
For HTTP APIs, breaking changes are managed through versioning in the URL path (see xref:021.adoc[TS-21]). For websites and web applications, content changes SHOULD NOT require a URL change; the URL identifies the resource, not a particular version of its representation.
