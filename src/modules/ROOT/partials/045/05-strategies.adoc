...
* *xref:garden::rollback.adoc[Rollback].* Re-enable the source. Cheap, provided the source was not modified.
...
* *xref:garden::rollback.adoc[Rollback].* Per-partition. A failed partition can be routed back to the source while others continue. This bounded rollback is the principal advantage of the phased approach.
...
2. *Dual-write.* Begin writing new and updated records to both source and target. The source remains the system of record; writes to the target are additive and must not block or fail the request.

3. *Backfill.* Copy existing source data into the target. The backfill runs in the background, in idempotent batches, and reconciles continuously with the dual-written stream so that the target converges on the source.

4. *Verify.* Compare source and target exhaustively (or by sampling) until they are demonstrably consistent.

5. *Switch reads.* Direct reads to the target, initially for a small fraction of traffic and then ramping. Both source and target continue to receive writes.
...
* *xref:garden::rollback.adoc[Rollback].* Each stage is independently reversible: reads can be repointed back to the source, dual-writing can be resumed, the source can resume its role as system of record. The cost of rollback rises sharply once "switch writes" has executed.
