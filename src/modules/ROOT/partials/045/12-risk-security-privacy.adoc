...
A migration plan that does not state the classification of its data is incomplete. See xref:053.adoc[TS-53].
...
The migration's credentials SHOULD be distinct from any long-lived application or operator credential, so that they can be revoked without affecting other work. See xref:052.adoc[TS-52].
