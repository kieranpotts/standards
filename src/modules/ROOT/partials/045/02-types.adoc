...
* *xref:garden::big-bang.adoc[Big-bang].* The source is quiesced, the migration completes, and the target is enabled in a single coordinated step. Simplest to reason about; highest downtime; highest blast radius on failure.
