...
For the general principles of observability, see xref:057.adoc[TS-57]. This section addresses what is specific to migrations.
...
* *xref:garden::throughput.adoc[Throughput].* Records per unit time, with a baseline so that degradation is detectable.
...
* *xref:garden::resource-utilization.adoc[Resource utilization].* CPU, memory, I/O, and lock contention on the source and target, attributed to the migration, so that contention with the live workload is detectable.
