...
In a token-based model, a server verifies credentials once, then issues a signed token — typically a JWT (see xref:056.adoc[TS-56] for token design) — that itself carries the claims needed to establish identity and permissions. The server does not need to persist anything: validation is a signature check, not a lookup.
...
Where a token-based model is used, access tokens SHOULD be short-lived — typically minutes, not hours — to bound the damage of a leaked token. A longer-lived refresh token, exchanged for new access tokens without requiring re-authentication, offsets the resulting UX cost. See xref:056.adoc[TS-56] for refresh-token design.
