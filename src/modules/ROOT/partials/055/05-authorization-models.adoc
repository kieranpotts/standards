...
Whichever model is used, the requirements set out in xref:052.adoc[TS-52] apply without exception: authorization decisions MUST be enforced server-side, MUST be checked before every operation, and MUST NOT be inferred from the absence of a client-side affordance.
