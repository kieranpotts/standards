...
* *OpenID Connect.* JSON/JWT-based, built on OAuth 2.0 — see <<oauth-2-0-and-openid-connect,OAuth 2.0 and OpenID Connect>>. The RECOMMENDED default for new integrations: simpler to implement correctly, smaller payloads, and native fit for mobile and single-page applications, which SAML was not designed for.
...
Role and permission assignment on first login SHOULD be driven by group or role claims asserted by the IdP where available, falling back to a configurable default role otherwise. See <<authorization-models,Authorization models>>.
