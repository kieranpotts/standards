...
This workflow rebases branches onto `dev` frequently — the RECOMMENDED cadence is at least once per day for temporary branches (see <<integrating-temporary-branches-into-dev,Integrating temporary branches into dev>>), and epic branches merge `dev` into themselves at regular intervals. Both patterns can require resolving the same conflict repeatedly: a rebase replays every commit on the branch against the new base, so a conflict in an early commit may need to be resolved again for each subsequent rebase, and a long-lived epic branch can hit the same conflicting hunk on every merge-down until the underlying divergence is finally reconciled.
