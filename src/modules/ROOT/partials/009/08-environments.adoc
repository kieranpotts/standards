...
* xref:garden::database-branching.adoc[*Database branching*] — Services like Neon, PlanetScale, and Supabase support per-branch database copies that pair naturally with per-branch application deployments, so each preview gets its own isolated data layer.
...
*Development integration environment* — A shared environment where the latest `dev` branch is automatically deployed. Used for:
...
The branch-to-environment mapping described above supports a range of deployment strategies (big bang, blue/green, canary, rolling, feature flags). The choice of strategy is covered in xref:010.adoc[TS-10].
