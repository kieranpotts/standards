...
Pull requests are a useful tool, but they are not a universal default. This standard prioritizes xref:garden::continuous-integration.adoc[*continuous integration*] over PR-gated review, because PR-gated workflows introduce significant costs:
...
The *trunk* and *branch* workflows described in <<Workflows>> allow continuous integration without PR gates. Quality is maintained by:
...
All pull requests MUST target the development trunk (`dev`, or `<version>/dev` in projects with parallel LTS pipelines — see <<long-term-support,Long-term support>>). New work does not enter the pipeline at any other point. The `test`, `ready`, and release branches are downstream of `dev` and are populated only via the automated promotion described in <<integrations-to-the-testing-and-ready-trunks,Integrations to the testing and ready trunks>>. PRs to those downstream branches MUST be rejected by repository configuration.
...
* *Rebase and fast-forward* — RECOMMENDED for integrating temporary branches (`temp/*`) into `dev`. This is consistent with the local Git CLI workflow described in <<integrating-temporary-branches-into-dev,Integrating temporary branches into dev>>. Temporary branches are short-lived and have clean, linear histories that are worth preserving as discrete commits on the trunk.
...
No merge configuration applies to spike branches (`spike/*`), since they are exploratory and never integrated into any trunk (see <<spike-branches,Spike branches>>). A spike MAY still have a draft PR opened against it as a forum for discussing the approach (see <<draft-work-in-progress-prs,Draft / work-in-progress PRs>>), but that PR is closed rather than merged.
...
A PR title becomes the commit message on the trunk when the PR is squash-merged. PR titles SHOULD therefore follow the same revision header conventions as commit subject lines (see <<commit-message-header,Commit message header>>): a lowercase type prefix, an optional scope, the BREAKING/INCOMPAT flag where applicable, and a concise imperative subject. For example: `behavior(auth): add OAuth device flow` or `fix: handle empty cart on checkout`.
