...
Repository ownership need not mean repository exclusivity. It is RECOMMENDED that internal reference repositories follow an *internal open source* collaboration model: any engineer in the organization can read, clone, and contribute to most other teams' repositories, with appropriate code ownership in place to route review and preserve accountability.

This is the same fork-and-pull mechanics described above for external contributors (see <<cloning-workflows,Cloning workflows>>), applied *within* the organization instead of across its boundary. An engineer outside the owning team forks or directly branches the repository, implements the change, and opens a pull request; the owning team's <<code-owners,Code owners>> configuration automatically routes the review to the people accountable for that code, and MAY require their approval before merge.

The model has real governance tradeoffs, which is why it is a recommendation rather than a requirement:

* *Benefits.* Contributors are not blocked waiting for the owning team's capacity; engineers build a broader understanding of the codebase beyond their own team's boundary; and small cross-cutting changes (a typo fix, a shared-library bump, a one-line bug fix) can be made by whoever finds them, rather than filed as a ticket and queued behind the owning team's other work.

* *Costs.* Reviewing an unfamiliar contributor's change costs the owning team time regardless of who wrote it; a codebase that is easy to change from outside is also easy to degrade from outside, without a code owner's domain knowledge; and broad write access widens the audience that MUST be trusted with credentials, secrets, and production-adjacent configuration that may live alongside the code (see <<secrets-and-pii,Secrets and PII>>).

Code ownership is the control that makes the tradeoff favorable: it does not restrict who can *propose* a change, but it does guarantee that someone accountable for the code reviews it before it merges. Without a code owners file in place, opening every repository to every engineer removes the gatekeeping that made repository boundaries meaningful in the first place. Organizations that are not ready to configure and maintain code ownership across their repositories SHOULD default to the narrower, team-scoped ownership model described above, and adopt the internal open source model incrementally, repository by repository, as code ownership is put in place.
...
In addition, developers SHOULD be able to check out _any prior version_ from a repository's history and be able to build, run, test, and deploy that version – without relying on any external dependencies that are not configured at the same version point in the same repository.
...
* For repositories that are not scoped to any particular project or team, and which are relevant to the whole organization, use a generic prefix like `common__`, `shared__`, or `global__`, or the name of the organization itself.
...
https://www.toptal.com/developers/gitignore[gitignore.io] provides curated `.gitignore` templates for most languages, frameworks, and operating systems; combine the relevant templates and adjust to the project's needs.

Particular care SHOULD be taken to exclude files that may contain secrets — `.env`, `.env.local`, credentials, key files. Even with secret-scanning hooks (see <<secrets-and-pii,Secrets and PII>>), `.gitignore` is the first line of defence against accidental commits.

== `.gitattributes` files

Every repository MUST include a `.gitattributes` file at the repository root. This file tells Git what settings to automatically apply to specific file types or paths — most importantly, line-ending normalization (see <<git-configuration,Git configuration>>), but also whether a file type is treated as text or binary, whether it should be excluded from archive exports, and whether it should be tracked via Git LFS.
...
`AUTHORS` is distinct from `CODEOWNERS` (see <<code-owners,Code owners>>): `AUTHORS` is about credit and attribution, while `CODEOWNERS` is about review routing and governance. A repository MAY have both.
