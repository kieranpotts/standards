...
* *Epic branches* (`epic/*`): shared between multiple developers, so rewriting pushed history MUST be coordinated with all contributors. The merge-down sync strategy described in <<epic-branches,Epic branches>> deliberately avoids the need for rebases on epic branches.
...
A *behavior* is a change in a user-facing operation of the software. This name matches how xref:001.adoc[TS-1] labels functional requirements. Behavior revisions will typically toggle an externally-facing operation, such as enabling a new API endpoint. This type of revision covers changes and extensions to existing behaviors, and the deprecation and removal of old ones.
...
****
Quality revisions and refactoring are both concerned with improving the _quality_ of the software, but they focus on different sets of quality attributes:

* Quality revisions address qualities like availability, consistency, latency, security, and compliance. These are _external_ quality attributes of a system, because they impact the user experience. They are also _dynamic_ quality attributes, because they tend to emerge at runtime. (Hence the `quality` commit type.)
...
The focus of refactoring work is on the needs of the developers and maintainers of the software, whereas the focus of quality revisions is on delivering better quality user experiences.
****

=== Refactor commit discipline

Refactoring is most effective as a deliberate two-step discipline, rather than as an incidental byproduct of feature work. Kent Beck's maxim captures the sequencing: first *make the change easy* – which may itself be hard – then *make the easy change*. Restructuring the code to accommodate a new requirement is separated out as its own step, completed and committed before the requirement itself is implemented, rather than entangled with it in a single commit.

This sequencing produces two atomic commits in place of one mixed commit: a `refactor` commit that reshapes the existing code without changing its behavior, followed by a `behavior`, `quality`, `fix`, or `step` commit that makes the now-easy change. This is the same discipline described for preparatory refactors discovered mid-task (see the <<atomic-commits,Atomic commits>> tip on stashing in-progress work), generalized into a default way of working rather than a fallback for when a refactor is discovered to be necessary partway through.

Applied consistently, a high proportion of a project's commits will be `refactor` commits – roughly half is a reasonable target. A commit log with few or no `refactor` commits is a signal that changes are being forced into code that was not first reshaped to accommodate them, which tends to produce larger, less atomic commits and code that accretes complexity around each new requirement instead of being continuously reshaped to fit it.

xref:007.adoc[TS-7] and xref:008.adoc[TS-8] cover the design and ticketing sides of refactoring work; this discipline is the version-control-level practice that keeps the resulting commits atomic and the history readable.
...
Finally, a *version* commit captures a set of changes made in preparation of a new software release, such as tagging the version number. It marks that a version has been cut, not that it has been deployed to production – see <<Releases>> for the distinction between a release candidate and an actual deployment. The remaining two commit types, *merge* and *revert*, are required to capture specific Git operations.

[NOTE]
======
Some of these commit types map directly to issue types, as defined in xref:008.adoc[TS-8]. For example, `behavior` commits will typically be associated with "feature" issues, and `fix` commits will typically be associated with "bug", "vulnerability", and "incident" issues. However, these technical standards do not mandate a one-to-one mapping between commit types and issue types. For example, a `refactor` commit may be associated with a "refactor" issue, but equally it may be associated with a "feature" issue, if the refactoring work is necessary to implement the feature.
======
...
Generally, the `<description>` SHOULD start with a verb describing the _action_ that is being taken by applying the commit. There are some exceptions. Bug fixes need only describe the _problem_ that is being fixed. And version commits can simply give the version number of the release.
...
The BREAKING flag MAY be used by automation tools to programmatically bump major version numbers (see xref:011.adoc[TS-11]), and to automate the generation of release notes, changelogs, and other such artifacts.
...
WIP commits MUST NOT be pushed to the trunk lines (`dev`, `test`, `ready`). They are permitted only on temporary or epic branches, and MUST be cleaned up (rebased, squashed, or amended) before integration into `dev`. See <<handling-work-in-progress-commits,Handling work-in-progress commits>>.
