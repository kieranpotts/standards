...
Where a document's shape matches a well-known, publicly-schema'd format (GitHub Actions workflows, Kubernetes manifests, and many others), the https://www.schemastore.org/json/[JSON Schema Store] provides a ready-made schema, and tooling that consults it can validate the file with no per-project configuration at all.
