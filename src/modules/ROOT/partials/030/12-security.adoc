...
* Treat any YAML document from outside the project's own trust boundary as untrusted input, and validate it against a schema (see <<schema-validation,Schema validation>>) before acting on its contents.
...
* Do not embed secrets directly in YAML files committed to version control. See xref:052.adoc[TS-52].
