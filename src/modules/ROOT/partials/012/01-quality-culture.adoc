...
* *Requirements clarity* — ambiguous or incomplete requirements are the single largest source of defects. Investing time in clear, testable requirements (see xref:001.adoc[TS-1]) directly reduces defect rates.
...
* *Test-driven development* — writing tests before implementation forces developers to think through requirements, edge cases, and interface design before writing production code. See xref:013.adoc[TS-13] for detailed guidance.
