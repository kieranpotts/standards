...
The DoD is more important than a Definition of Ready (DoR). While a DoR describes the conditions under which work _may begin_, the DoD describes the conditions under which work _has ended_. The DoD is also more stable — it tends to remain consistent across work items, whereas the DoR often varies depending on the nature and complexity of each task.

The DoR is principally concerned with requirements readiness, and so is specified separately — see xref:001.adoc[TS-1].
...
A DoD reduces how much technical debt accumulates silently, but it does not eliminate debt the team takes on knowingly — a shortcut accepted under deadline pressure, a migration left half-finished, a dependency pinned to an old major version. That debt still needs to be tracked and prioritized, and "pay it down eventually" is not a prioritization scheme.

Classify each known item of technical debt into one of three types:

1. *Blocking now.* The debt is already slowing delivery or causing defects — a fragile module that breaks on every unrelated change, a manual step that delays every release. This category SHOULD be minimized aggressively: schedule the work rather than deferring it further, because its cost is being paid on every iteration until it is addressed.

2. *Will block later.* The debt does not yet slow delivery, but a known, foreseeable event will make it a blocker — a library reaching end of life, a data volume the current design will not scale to, a workaround for a platform limitation that is due to be removed. This category deserves the team's primary planning attention: it is cheaper to schedule the work before the blocking event than to firefight it afterward.

3. *Might block later.* The debt is speculative — it could become a problem under circumstances that may never arise. This category SHOULD generally be left alone. Tracking every item of hypothetical debt and re-litigating it at each planning cycle is itself a cost, and most speculative debt never matures into an actual blocker.

Record each item of tracked debt in the issue tracker (see xref:008.adoc[TS-8]), tagged with its classification, so that "will block later" items surface at planning time rather than being rediscovered under pressure when the foreseeable event finally arrives.
