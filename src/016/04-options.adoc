...
file paths you provide to `cp` are arguments. Their order is significant: `cp
foo bar` is not equivalent to `cp bar foo`. (Arguments look similar to, but are
distinct from, subcommands. A command cannot have both arguments and
subcommands.)
...
eg. `--no-recursive`. * `-<x>`, where `<x>` is a single-letter abbreviation, eg.
`-r`
...
change in the lifetime of the tool. Good examples include `cp [source]
[destination]` and `rm [file1] [file2] ...`.
...
* https://specifications.freedesktop.org/basedir-spec/basedir-spec-latest.html[Full
specification]
...
https://pubs.opengroup.org/onlinepubs/009695399/basedefs/xbd_chap08.html[listed
here].
...
* Environment variables in systemd units are globally readable via `systemctl
show`.

As explained in the section on arguments and flags, best practice is to accept
secrets only via files and stdin.
