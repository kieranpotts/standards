...
The following serves as a useful starter template for reporting the outcomes of a threat modeling workshop. It incorporates patterns from threat modeling frameworks such as STRIDE and PASTA. See also link:./examples/threat-modeling-workshop.adoc[this concrete example of a threat modeling workshop report], which is based on the same template.
