...
** *Other stakeholders*: Tom Wilson (DevOps Lead), Rachel Green (Compliance
Manager)
...
1. *Internet → API gateway*: transition from untrusted public internet to
semi-trusted DMZ.
2. *API gateway → internal services*: transition from DMZ to trusted internal
network.
3. *Application layer → data layer*: transition from application services to
data persistence.
4. *Internal network → AWS services*: transition from self-managed
infrastructure to AWS-managed services.
5. *internal network → third-party services*: boundary with integrated services
(Auth0, Stripe).
...
*Next Review Date:* 2026-04-16 (quarterly review) or upon major architecture
changes
