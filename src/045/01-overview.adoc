...
* _Data migration_ is the transformation and copying of data from source to target. It is a batch or streaming operation that runs against the data, not against the live system's request path. It can be paused, resumed, re-run, and validated independently.

* _Switch-over enablement_ is the act of directing the live system — its reads, its writes, or both — to the target representation instead of the source.
...
It does not cover routine data _ingestion_ or ETL pipelines whose purpose is to feed downstream analytics, nor does it cover backup and restore as a disaster-recovery activity — though the techniques overlap. For release mechanics around the switch-over, see *link:../010/README.adoc[TS-10: Releasing]*.
