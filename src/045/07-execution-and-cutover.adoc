...
1. *Read switch.* Direct a fraction of read traffic to the target. Ramp up. Both source and target remain writable.

2. *Write switch.* Stop writing to the source. The target becomes the system of record.

3. *Soak.* Run with the target as system of record, with the source preserved as a fallback, for a defined period — typically days, not hours.
...
For release mechanics — feature flags, canary rollout, and rollback orchestration — see *link:../010/README.adoc[TS-10: Releasing]*. For the runbook as a documentation artifact, see *link:../025/README.adoc[TS-25: Technical Documentation]*.
