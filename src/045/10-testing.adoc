...
Before a zero-downtime migration runs against production, the dual-write path SHOULD be exercised in a _shadow mode_: writes are duplicated to the target, but the target is not read by the live system and the duplication cannot affect the request path. Shadow running surfaces dual-write bugs — out-of-order writes, retry hazards, idempotency failures — under real load, before the target is relied upon.

For big-bang and phased migrations, a _dry run_ against a full copy of the source SHOULD be performed end-to-end, including validation, to confirm that the migration completes within its budget and produces a validated target.
...
For testing practices in general, see *link:../012/README.adoc[TS-12: Quality Assurance]* and *link:../013/README.adoc[TS-13: Functional Testing]*.
