...
Non-functional requirements – aka. quality requirements (QRs) or cross-functional requirements (CFRs) – define the operational constraints within which the software system is expected to work. These are the system's dynamic quality attributes. They include, but are not limited to, security, performance, scalability, availability, and data retention requirements.
