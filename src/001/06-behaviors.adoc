...
*Behaviors* are the functional requirements (FRs) of a software system: what it does, from the perspective of those who interact with it. (How _well_ it does those things at runtime — its dynamic non-functional requirements — is covered separately under <<_qualities>>.)

The behaviors of a system are best documented across five complementary sections, each giving one kind of functional requirement a single home:

* *Access* — the permission matrix: which <<_context, actors>> may exercise which capabilities.
* *Rules* — invariants, policies, and entity lifecycles that hold across many features.
* *Features* — concrete, scenario-level behaviors, written as testable Gherkin scenarios.
* *Journeys* — how features combine into end-to-end flows.
* *Interfaces* — the external contract: the operations, resources, and events the system exposes.

== Access

The *access* section is the authoritative map of which actors may exercise which capabilities of the system. Where <<_context, actors>> describes _who_ the participants are, access states _what each is permitted to do_ — and because a permission is something the system must enforce, it is a requirement, not mere description.

Permissions are inherited down the <<_context, actor hierarchy>>: a capability available to a lower-privileged actor is available to all higher-privileged actors. State each capability once, against the lowest-privileged actor that holds it. A *permission matrix* (capability × actor → allowed?) is the single place to read off the complete permission picture:
...
Individual <<_features, features>> specify the behavior of each capability; <<_rules, rules>> govern any finer-grained conditions.

== Rules

The *rules* section documents the business rules, invariants, and policies that hold across the system — the constraints not tied to any single feature but applying wherever the relevant entities and operations appear. Capturing them once keeps them authoritative: a feature scenario references a rule by its identifier rather than restating it, so each invariant lives in exactly one place.

State each rule so that it is unambiguous and, where possible, testable, and give each a stable identifier (eg. `R1`, `R2`) so it can be referenced from elsewhere. For example:

* *R1 — Single status.* A record has exactly one `status` at any time. It is never in more than one state at once.

Rules also document the *lifecycle* of domain entities — the states a <<_context, model>> entity can hold and the transitions permitted between them. The model defines the vocabulary of states; the rules define the legal moves between them, and who may trigger each. A state-transition diagram is a good way to express a lifecycle.

== Features

The *features* section documents the concrete, scenario-level behaviors of the system. Features are organized by actor: because privileges are inherited up the <<_context, actor hierarchy>>, each feature is specified once, against the lowest-privileged actor that can exercise it, and is thereby available to all higher-privileged actors. The complete map of which actor may exercise which capability is recorded in the <<_access, access matrix>>.
...
The *journeys* section shows how the system's <<_features, features>> combine into coherent end-to-end flows — the path a user or consumer follows to accomplish a goal. Whereas features specify _what_ the system does, journeys show _how_ those functions come together. The form depends on the system:

* For *user-facing systems*, journeys are wireframes and visual designs — the screens a user moves through. Keep them at the level of structure and flow; pixel-perfect visual design is a downstream concern and does not belong in the requirements specification.

* For *headless systems*, journeys are annotated call-sequences — the ordered operations a consumer invokes to achieve a goal, with the significant inputs and outcomes at each step.

Journeys apply only where there is a multi-step flow worth showing. A system whose goals are each met by a single operation has no journeys to document.

== Interfaces

The *interfaces* section describes the system's external contract — the operations it exposes, the resources they act on, and any events it emits, described in terms of _what each does_, not how it is implemented.

This is the _behavioral_ contract only. Protocol, transport, payload formats, and endpoint naming are technical design decisions, recorded separately (see *link:../003/README.adoc[TS-3: Design Docs]* and the companion RFC repository). State here only what a consumer can rely on: the operations available, their inputs and outputs in <<_context, domain>> terms, and the <<_context, actors>> permitted to invoke them (per <<_access, access>>).
