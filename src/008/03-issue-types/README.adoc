...
An issue may or may not be small enough to be implemented in a single *atomic commit*. If not, a *temporary branch* MAY be used to implement the changes over multiple commits. See *link:../../009/README.adoc[TS-9: Version Control]*.
...
|PERFORMANCE
|A change to a constraint within which the system must perform
|`#008800`
...
include::./08-performance.adoc[leveloffset=+1]
...
└─ YES
   ...
   │  └─ NO ────┐
   │            │
   ├─ Is this about performance or efficiency?
   │  │
   │  ├─ YES ─── *PERFORMANCE*
   │  │
   │  └─ NO ────┐
...
|Small step as part of a larger performance initiative
|DEV
