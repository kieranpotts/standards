...
While features capture the implementation of functional specifications, *performance* revisions capture the implementation of non-functional requirements. The term "performance" is used broadly here, to refer to all operational qualities, also known as dynamic quality attributes, such as security, responsiveness, reliability, resilience, and so on.

Both features and performance optimizations are user-facing changes. However, not all performance optimizations will be observed by users in any kind of quantifiable way. For example, the implementation of more concurrent processing may reduce the operational costs for the software vendor, rather than reduce latency for users. The defining characteristic of performance revisions is that the change is observable and measurable externally of the system at runtime.
...
Typically, there would be a number of incremental changes to code and configuration before a feature, performance optimization, or fix is complete. These *steps* compose the building blocks for larger changes, or they explore possible design paths in an iterative fashion. Individually, these commits do not change user-facing operations or performance of the software, but they do represent small increments toward such changes being enabled.
...
****
Performance optimization and refactoring are both concerned with improving the _quality_ of the software, but they focus on different sets of non-functional requirements:
...
The focus of refactoring work is on the needs of the developers and maintainers of the software, whereas the focus of performance optimization is on delivering better quality user experiences.
****
...
#   - merge        An explicit merge commmit.
#   - performance  Optimization of the system's runtime performance.
