...
Clear and structured commit messages make an important contribution to the internal quality of a software system. The output of `git log` is a valuable artifact in its own right. Most crucially, it is the source-of-truth for understanding the evolution of a codebase over time.

A repository's commit history is also likely to endure longer than other artifacts such as tickets (issue trackers may be replaced) and decision logs (which have a tendency to grow stale over time).

Consistent commit message formats support greater automation of recurring development and operations tasks. Detailed commit messages also make tools like `git blame` more useful. Clean commit logs support multiple objectives: automation (enabling tooling to parse and act on commit metadata), quality control (through atomic commits that don't break the build), and provenance (creating a machine-parsable changelog linked to business requirements).

Good commit hygiene greatly improves the maintainability of a software system. You SHOULD commit deliberately and strategically, with the same care and attention to detail as given to the code itself. _Clean commits_ are as important as _clean code_.

== Atomic commits

A clean, searchable commit log is formed from atomic commits.

An atomic commit is a small, self-contained, incremental change to a codebase that does not break the build or fail any tests. An atomic commit does not necessarily represent a "complete" feature, bug fix, refactor, upgrade, or performance optimization, but it does represent a small logical step toward one of those outcomes.
...
But atomicity is not only about small commits. Atomicity is several constraints: small, stable, and self-contained.
...
* *Epic branches* (`epic/*`): shared between multiple developers, so rewriting pushed history MUST be coordinated with all contributors. The merge-down sync strategy described in *link:./branches.adoc[]* deliberately avoids the need for rebases on epic branches.
...
A *feature* is a change in a user-facing operation or behavior of the software. Features will typically toggle an externally-facing operation or behavior, such as enabling a new API endpoint. This type of revision covers changes and extensions to existing features, and the deprecation and removal of old features.

While features capture the implementation of functional specifications, *runtime* revisions capture the implementation of non-functional requirements. The commit type is named "runtime" because it concerns the system's *dynamic* quality attributes — those that emerge at runtime and are observable externally — such as latency, throughput, availability, security, responsiveness, reliability, resilience, and so on. Performance in the strict sense (speed and capacity) is just one of these, not the whole of it.

Both features and runtime revisions are user-facing changes. However, not all runtime revisions will be observed by users in any kind of quantifiable way. For example, the implementation of more concurrent processing may reduce the operational costs for the software vendor, rather than reduce latency for users. The defining characteristic of runtime revisions is that the change is observable and measurable externally of the system at runtime.
...
Typically, there would be a number of incremental changes to code and configuration before a feature, runtime revision, or fix is complete. These *steps* compose the building blocks for larger changes, or they explore possible design paths in an iterative fashion. Individually, these commits do not change user-facing operations or runtime qualities of the software, but they do represent small increments toward such changes being enabled.
...
A *refactor* is any improvement to the design or internal structure of the code or configuration of the software, without changing the software's features or degrading its performance. Refactoring work includes changes to automated tests and build scripts, as well as to source code and configuration, and the data structures and data flows, of the software itself.

.Runtime revisions versus refactoring
****
Runtime revisions and refactoring are both concerned with improving the _quality_ of the software, but they focus on different sets of quality attributes:

* Runtime revisions address qualities like availability, consistency, latency, security, and compliance. These are _external_ quality attributes of a system, because they impact the user experience. They are also _dynamic_ quality attributes, because they tend to emerge at runtime. (Hence the `runtime` commit type.)

* Refactoring is about making improvements to the _internal_, _static_ qualities of a system – things like choices of languages, design patterns, data structures, and coding conventions. Typically, these qualities are observable at or before compile time, and they impact developers more than they do users.

The focus of refactoring work is on the needs of the developers and maintainers of the software, whereas the focus of runtime revisions is on delivering better quality user experiences.
****

A *format* commit applies presentation-only changes to code – updates to whitespace, indentation, line wrapping, or code style. These changes affect the appearance of the code without altering its structure, logic, or behavior. Formatter runs (eg. `prettier`, `black`, `gofmt`) and bulk code-style adjustments belong here, distinct from `refactor` commits, which restructure logic, data, or design. Keeping pure reformatting in its own commit type also makes it easier to filter such commits out when reviewing history with tools like `git log` and `git bisect`.
...
[NOTE]
======
Some of these commit types map directly to issue types, as defined in *link:../008/README.adoc[TS-8: Issue Tracking]*. For example, `feature` commits will typically be associated with "feature" issues, and `fix` commits will typically be associated with "bug", "vulnerability", and "incident" issues. However, these technical standards do not mandate a one-to-one mapping between commit types and issue types. For example, a `refactor` commit may be associated with a "refactor" issue, but equally it may be associated with a "feature" issue, if the refactoring work is necessary to implement the feature.
======
...
But version control repositories are widely used for other kinds of non-executable artifacts, such as documentation, requirements specifications, and staff handbooks. For these repositories, the following revision types are RECOMMENDED:
...
An *add* commit introduces new content – new documents, sections, or substantial new material that did not previously exist.

An *edit* commit improves existing content – rewrites, expansions, clarifications, or refreshes.

A *fix* commit corrects typos, grammatical errors, broken links, or factual mistakes. The key distinction from edits is that a fix addresses something that is broken or incorrect, whereas an edit improves something that is not, strictly, wrong.

A *remove* commit deletes outdated or redundant content that is no longer needed.

A *restructure* commit reorganizes, reorders, or renames content without changing its meaning.

A *format* commit applies presentation-only changes – updates to markup, style, or formatting. These are changes that affect the rendered appearance of the document without changing the underlying content.

A *chore* commit covers repository housekeeping – updates to CI configuration, tooling, ignore files, and other infrastructure that supports the maintenance of the repository but is not part of repository's core content.

Finally, *release*, *merge*, and *revert* commits carry the same meaning as in software repositories, as they represent Git operations rather than content concerns.
...
Typically, the BREAKING flag will be most commonly applied to features, but this is not a requirement.

The BREAKING flag MAY be used by automation tools to programmatically bump major version numbers (see *link:../011/README.adoc[TS-11: Versioning]*), and to automate the generation of release notes, changelogs, and other such artifacts.

While the BREAKING flag marks changes that are incompatible with external client applications, the *INCOMPAT* flag is used to mark _internal breaking changes_. An internal breaking change is, for example, a change to a function signature that requires changes to all calling code. Incompatible changes may also be done to data structures, database schema, message and event schema, and facades to dependencies and third-party systems.
...
WIP commits MUST NOT be pushed to the trunk lines (`dev`, `test`, `ready`). They are permitted only on temporary or epic branches, and MUST be cleaned up (rebased, squashed, or amended) before integration into `dev`. See *link:./workflows.adoc[]*.
...
While the header line describes _what_ changed (summarizing the commit's diff), the commit body goes into detail about the _why_.
...
****
Issue numbers SHOULD NOT be encoded in the subject line of commit messages. Instead, encode this information in footers. Once changes have been integrated and the issue is closed, the original requirements of the issue have effectively been transferred into the code, configuration, and other artifacts encapsulated in the repository, making the issue identifier less significant for future readers of the commit log. The footers provide a precise, machine-parsable binding without cluttering the subject line.
...
https://git-scm.com/book/en/v2/Customizing-Git-Git-Configuration#_commit_template[Here's how to configure a custom commit template] and below is a template you can use. Be sure to include the empty line at the top – this is where the cursor will be placed when the user's editor is opened.
...
#   - format       Low-level code formatting changes.
