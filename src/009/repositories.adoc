...
Dependencies SHOULD NOT be committed to version control. Bloating Git with vendor libraries and other binary artefacts slows clone operations and is poorly suited to Git's strengths.

Instead, use a *binary repository manager* — such as JFrog Artifactory, Sonatype Nexus, or AWS CodeArtifact — to host and proxy your dependencies. Combined with package *lock files* (eg. `package-lock.json`, `poetry.lock`, `Cargo.lock`), which pin exact versions and MUST be committed to version control, this approach guarantees reproducible builds without bloating Git. Binary repository managers also protect against upstream registries changing or disappearing — a real risk for long-lived projects.
...
Particular care SHOULD be taken to exclude files that may contain secrets — `.env`, `.env.local`, credentials, key files. Even with secret-scanning hooks (see *link:./security.adoc[]*), `.gitignore` is the first line of defence against accidental commits.
...
`AUTHORS` is distinct from `CODEOWNERS` (see *link:./pr-config.adoc[]*): `AUTHORS` is about credit and attribution, while `CODEOWNERS` is about review routing and governance. A repository MAY have both.
