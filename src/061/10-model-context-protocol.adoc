...
MCP is a client-server protocol. An *MCP server* exposes a set of tools and resources. An *MCP client* — typically an agent harness or other AI application with a model at its core — connects to one or more MCP servers and uses their tools to fulfill tasks.
...
Indiscriminately connecting many MCP servers will therefore degrade performance (through context rot) and increase cost (through per-token billing).
