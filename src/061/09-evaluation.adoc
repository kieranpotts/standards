...
For properties that can be verified mechanically — eg. regex matches, passing tests — your evals should make objective assertions. For more qualitative properties — eg. clarity, tone — you can either use an LLM as the judge (the evaluating model MUST be run in a distinct session from the one that generated the output being evaluated) or apply human judgment.
