...
Beyond a handful of evals implemented as ad-hoc scripts, it is RECOMMENDED to use an established eval framework — such as https://github.com/openai/evals[OpenAI Evals], which pairs a runner with an open registry of reusable evals. For evaluating skills, Anthropic's https://github.com/anthropics/skills/tree/main/skills/skill-creator[`skill-creator`] skill automates much of the evaluation and iteration loop to refine the effectiveness of a new skill.
...
For properties that cna be verified mechanically — eg. regex matches, passing tests — your evals should make objective assertions. For more qualitative properties — eg. clarity, tone — you can either use an LLM as the judge (the evaluating model MUST be run in a distinct session from the one that generated the output being evaluated) or apply human judgment.
