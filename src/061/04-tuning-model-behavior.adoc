...
Reasoning models expose a separate dial, which is how much compute the model spends "thinking" — ie. producing an internal chain of thought — before committing to a final answer.

This is orthogonal both to temperature and to model tier. A capable model with its thinking budget turned down may underperform a smaller one with it turned up, on the right task.
