...
Most hosted model services are metered. Hosted models are typically billed per token, charging separately for input and output — and the two are not priced equally. Output tokens typically cost several times more than input tokens. Reasoning ("thinking") tokens are treated as output, and billed as such. Meanwhile, cached input tokens, where supported, are billed at a steep discount — often roughly an order of magnitude cheaper than uncached input.
...
The total cost of a task is this calculation for each inference call made in a loop until the success criteria are satisfied.

Some hosting providers will apply a multiplier to the total cost for inference via their flagship frontier models.
...
The largest cost lever is usually model choice. Defaulting to the biggest model for every task is wasteful. Defaulting to the cheapest is unreliable and may, ultimately, cost more due to the need for rework.
...
The smallest high-signal context is therefore both the most effective and the cheapest. Context engineering best practices, covered elsewhere in this technical standard, are therefore also cost-reducing strategies:
...
Prompt caching is where a prompt prefix can be reused across inference calls at a large discount. Not all model hosting providers offer this, but some do, and it is one of the highest-leverage levers for multi-turn sessions and agentic loops, where the same system prompt, skills, and reference context are sent repeatedly.
...
* Batching similar tasks into one session amortizes shared context across them. Do this sparingly, however. The trade-off for batching is you end up with large diffs to review.
