...
Most AI coding tools provide mechanisms to reuse context across sessions. The exact naming varies between tools: *instructions*, *rules*, *skills*, *commands*, *custom prompts*, etc. Whatever the name, all these systems share the same underlying purpose, which is to load reusable bundles of context, either at the start of a session (statically), or on-demand under predefined conditions (dynamically).

Bundles of reusable context help to improve consistency in AI-assisted and agentic workflows by encoding programming conventions, architectural decisions, and domain knowledge into artifacts that reload automatically in fresh sessions.

== General best practices

Reusable context consumes tokens, depleting a model's attention budget and so competing with the user's actual prompt. Thus, the same core principles of context engineering apply:
...
* Prefer just-in-time retrieval (see below) over always-on loading. For example, rather than embedding a full coding standard in a context bundle, instead provide a pointer to it and let the agent load it on-demand.
...
* If you're using capable, frontier coding models, you will find you will get little value from using context bundles to specify universal best practices like "never commit secrets" or "match the prevailing code style". Such knowledge will already be embedded in your model. Instead, use context bundles to capture guidelines, standards, and requirements that are specific to your project, such as "minimum 75% unit test coverage" and "conforms to neostandard linter defaults".

== Conventions

As of 2026, AI tool makers have not come to a consensus on where reusable context should live, what to call it, how to structure it, or how to load it. The consequence is that different agent harnesses have different conventions for discovering and loading reusable context.

If you or your team use multiple agent harnesses, or if you expect to switch between them over time, then you may find yourself maintaining the same bundles of context in artifacts like `CLAUDE.md`, `.cursorrules`, `.github/copilot-instructions.md`, and so on. This will quickly become unsustainable. Duplication will lead to drift, and drift will lead to subtle inconsistencies in behavior between agents.

Therefore, it is RECOMMENDED to maintain a single source-of-truth collection of reusable context bundles, and to transform this source material into distributable artifacts in formats tailored for the different agent harnesses that you use.

Despite the current proliferation of conventions for reusable agent context, the industry is slowly converging on two standards: `AGENTS.md` and skills files. It is RECOMMENDED to adopt these standards for your source files, from which you compile and distribute artifacts for different agent harnesses.
...
The "repository structure" section is RECOMMENDED. It's one of the highest-leverage things you can give an agent, sparing it from wasting tokens grepping around the project's directory tree.

Other sections you might consider including in `AGENTS.md` include "code formatting conventions", "design patterns", "testing instructions", "pr creation", and "security standards".

Start with a minimum baseline then extend your agent instructions incrementally to improve output.

== Agent skills

Skills are a complementary open convention for packaging reusable bundles of context that agents can load on demand.

Skills complement `AGENTS.md`. Whereas `AGENTS.md` provides a static, always-loaded orientation to the project as a whole, skills encapsulate specific procedures, standards, runbooks, or playbooks that are dynamically loaded when they are relevant to the current task an agent is undertaking.

The canonical specification lives at https://agentskills.io/specification.

=== Skill scope

An emerging best practice is to use skills to define a small set of common workflow steps, while keeping knowledge (standards, policies, constraints, domain models, etc.) as separate reference documents that are loaded on-demand from other sources.

Workflow skills — "planning", "coding", "testing", "deploying", "releasing" — should be step-by-step, deterministic instructions, free of domain-specific facts, so they rarely change between projects and domains. By comparison, reference materials that encapsulate standards, policies, constraints, and domain models are dynamic and volatile.

This separation makes workflow skills more reusable. A skill that only says _how_ to look something up and what to do with it, rather than hard-coding the answer, stays technology-agnostic and domain-agnostic, and runs unmodified against any project that supplies its own reference material on demand.
...
Pulling knowledge dynamically also helps to keep your agentic workflows evergreen. And enforcing this separation of concerns will also help you to debug issues with your agents, because when something goes wrong you can ask:
...
In particular, a skill SHOULD NOT combine _evaluation_ and _implementation_ in one responsibility. A skill that analyzes and reports findings (a review, an audit, a proofread) should be distinct from a skill that enacts a change (a fix, a migration, a refactor). These two concerns SHOULD NOT be bundled into a single skill. For example, a skill that reviews a pull request SHOULD NOT also be the skill that applies the fixes it recommends.
...
A skill SHOULD be explicit about its inputs and outputs, in the same way a well-designed function is explicit about its parameters and return value. A skill file should explicitly define the following:
...
Non-interactive skills can be run unattended, including by parallel sub-agents — so these types of skills should be preferred by default, as they are inherently more reusable.

Reserve interactive skills for cases where human interaction _is_ the value the skill provides — a structured discovery interview, for example, where the dialogue itself is the point.

This preference for non-interactive skills reflects a broader industry trend sometimes called the _specs-to-code_ movement. This involves pushing interactive, human-in-the-loop work upstream, into requirements-gathering and specification, so that downstream delivery can run non-interactively from executable acceptance criteria, with minimal human involvement further down the pipeline.

=== Skill discovery

As mentioned earlier, there is currently lots of inconsistency in how agent harnesses discover and load reusable context bundles. Most agent harnesses will load project-specific skills, but the expected location and naming conventions of the files varies. The formal specification for agent skills defines the path to discover skills as `.agents/skills/`, relative to the root of the current project repository. But not all harnesses support this path out-of-the-box.

Some agent harnesses will also look for global skills in locations in the user's home directory. This is really useful for sharing common workflow skills across multiple projects. But support is patchy. As of 2026, only a few harnesses like Pi support the automatic discovery of global skills.

For now, until our industry has converged on universal standards, we must tailor the location and naming of our skills to adhere to the conventions of our chosen agent harnesses. See the https://github.com/JuliusBrussee/caveman/[Caveman] project for an example of how to install the same set of source skills into locations and formats for auto-discovery by multiple mainstream agent harnesses.

One option is to use `AGENTS.md` to reference the location of your skills files, and to provide instructions to agents to load these into context, just-in-time, when certain conditions are met. See the `AGENTS.md` template, above, for an example. It shows how to list both local skills (via relative paths, unique to the project) alongside global skills (via absolute URLs, reused across projects).
...
There's nothing stopping you including other artifacts not defined by the Agent Skills specification, as long as those artifacts are references from the `SKILL.md` entry point.

This technical standard RECOMMENDS including a `README.md`, for documenting the skill's use for the benefit of its human maintainers.
...
The `description` metadata field is the most important part of a skill. When an agent auto-discovers skills, it typically loads only the metadata from the skills files into its context — specifically, the `name` and `description` fields. The `name` field defines how users may explicitly invoke a skill, while the `description` field defines the conditions under which agents themselves should automatically load the rest of a skill into context.

The following principles help to produce effective skill descriptions, to ensure that the right skills are automatically loaded in the expected scenarios:
...
Remember that different agent harnesses will handle skill loading in different ways. The Agent Skills specification targets _on-demand_ activation. Conforming agents should read each skill's `description` field and decide whether to load it based on the current task. But some harnesses also support _always-on_ loading, where the skill is injected at the start of every session, and _manual_ loading, where the user triggers the skill via a slash command.

When writing a skill intended for broad deployment across multiple agent harnesses, check how each target harness resolves and activates skills. If your harness persists in context all auto-discovered skills, then you will need to be extra vigilant in designing your skills for token efficiency.
...
Aim for moderate detail rather than exhaustive coverage. Overly comprehensive skills can hurt more than they help. The agent may struggle to extract what is relevant and may pursue unproductive paths. Concise stepwise guidance with a working example tends to outperform exhaustive documentation. When you find yourself wanting to cover every edge case, consider whether most are better left to model judgment.

=== Creating skills

Effective skills are grounded in real expertise. Two approaches to creating new skills are RECOMMENDED:

*Extract knowledge from a hands-on task.* Complete a real task in conversation with an agent, providing context, corrections, and preferences along the way. Pay attention to the sequence of actions that led to success, corrections you made, input and output formats, and project-specific context you had to supply that the agent didn't already know.

Then crystallize that experience into a new skill for reuse next time you tackle the same problem.
...
A skill synthesized from your team's actual incident reports will outperform one generated from a generic "best practices" article, because it will capture your specific failure modes and recovery procedures.

*Have the agent draft its own skill.* An agent can be instructed, at the end of a session, to distill what it just learned — a workflow it worked out, corrections it received, project-specific facts it had to be told — into a new or updated skill. This is a good way of capturing reusable context that would otherwise be lost when the session ends, and it complements the two approaches above rather than replacing them: the agent has direct access to the session's corrections and context in a way that a human reconstructing it from memory does not.

Models tend to over-specify when drafting skills this way, producing verbose output that hedges, restates the obvious, and covers edge cases that will never occur. Treat an agent-drafted skill as a first draft, not a finished artifact. Proofreading and editing it down — cutting anything that fails the "would the agent get this wrong without this instruction?" test — is a key skill in its own right, and is where most of the value is added once the agent has done the initial extraction.
...
Here's a baseline template for a `SKILL.md` file. The two most important sections are:
...
The https://www.skills.sh/[Agent Skills Directory] (skills.sh) is the primary public registry for discoverable, installable skills. Skills listed there can be installed into any project via `npx skillsadd <owner/repo>`.
...
* *Provide working examples.* Public audiences have less context than your internal team. A small set of concrete input/output examples will significantly reduce the friction of adoption.
