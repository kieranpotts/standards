...
A solution is said to be "correct" if it meets both its functional and performance acceptance criteria. See also *link:./008-functional-testing.adoc[TS-8: Functional Testing]*, which covers best practices for the verification of a software system against its functional requirements, and *link:./009-performance-testing.adoc[TS-9: Performance (Non-Functional) Testing]*, which covers best practices for verifying that software meets its performance requirements.
