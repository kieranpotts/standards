...
In Unix-like environments, the primary output from most CLI programs goes to stdout. By default, this should be a human-readable text stream. Flags MAY support toggling of the output to a machine-readable format, for example, use `--plain` to support piping into tools such as `grep` or `awk`, and `--json` for use with `jq`.

Something SHOULD be sent to stdout on success, even if it is just a simple confirmation message that the operation has finished successfully. Some commands such as `cp` don't print anything, which has come to be regarded as bad practice. The user SHOULD be given explicit feedback on all operations.
