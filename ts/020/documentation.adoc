...
For public applications, the online documentation MUST be indexable by public search engines like Google.
...
If a command or sub-command requires one or more arguments or flags, you SHOULD design the command to return help text by default, when no arguments or flags are provided. So `my-tool cmd` would produce an abbreviated version of the output from `my-tool cmd --help` and `my-tool cmd -h`.
