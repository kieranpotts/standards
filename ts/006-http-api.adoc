...
As for what that structure should look like, there is no universal standard. There have been some attempts to standardize JSON API structures, notably the Open Data (OData) protocol (https://www.odata.org/), which is the closest thing we have to an industry standard for application-level messaging in JSON. But OData is an overly complex system for most use cases, and for this reason it is not widely adopted. Lighter weight standards include JSON API (https://jsonapi.org/) and JSON RPC.

An appropriate schema SHOULD be designed to meet the specific needs of the API. However, the following is a RECOMMENDED starting point. It is based on the JSON API standard, but is not compatible with it.

// TODO: The JSON Schema is incomplete.
.JSON Schema
[source,json]
----
{
...
----
{
      ...
      "items": [
        {
          ...
          "metadata": {
            "created_at": "2014-01-13T07:23:15Z",
            "updated_at": "2014-01-13T07:23:15Z"
          },
          "related": [
            {
  ...
  "metadata": {
    "cached_at": "2014-01-13T07:23:15Z",
    "keep_until": "2014-01-13T07:53:15Z",
  },
  "links": [],
  "messages": {
    "errors": [],
...
The "links" and "messages" fields MAY be included in HTTP response messages but MUST NOT be accepted by servers when included in clients' HTTP request messages. The "metadata" field MAY be included in both HTTP request and response messages. The "data" field MUST be included in both HTTP request and response message that have a payload.

==== "metadata"

The "metadata" field encapsulates metadata about the response or request. This metadata is not part of any representations of resources included in the "data" field, but it MAY hold additional information about those resources.

The "metadata" field is an object, which MAY be empty.

The "metadata" field MAY include any number of key-value pairs. The keys SHOULD be strings. The values MAY be any native JSON type, including other objects and arrays.

// TODO: Extend this section.

// TODO: Metadata can also be used to provide more granularity about the status of the response message, where HTTP status codes do not provide sufficient detail. For example, a `status` field could be included in the metadata to provide a more detailed status code than the HTTP status code alone. Alternatively reasons for the status code could be included in the "messages" field. This is particularly useful for error responses, where the client needs to know why a request failed.

HTTP message content SHOULD NOT duplicate information that is already provided in the HTTP headers. For example, the `Content-Type` header SHOULD be used to indicate the media type of the response body, and the `Content-Length` header SHOULD be used to indicate the length of the response body. The response body SHOULD NOT replicate this information. But you MAY include this information, and related information, in the "metadata" field if it is significant to the client _application_ (rather than being required by the web server or HTTP client library to process the HTTP message into an object representation).

==== "resources"

The "resources" field encapsulates the main resource representations within an HTTP message. It MAY be empty.

// TODO: The value of the "resources" field MUST conform to the following JSON Schema:

// NOTE: The naming convention for resources fields (`{field}`) MAY differ from the naming convention for other properties of the response structure. For example, a resource field might be named `FirstName`, while a property of the response structure might be named `total_items`.

==== "links"

// TODO: See the *Hypermedia* section, below, for more information on the structure of embedded hypermedia links.

==== "messages"

// Arrays of resources SHOULD be returned in a field named `items`. Metadata fields such as `total_items` and `total_pages` SHOULD be included in a `metadata` object.
...
It is strongly RECOMMENDED that response content includes hypermedia links. This has numerous benefits, including easier discoverability of resources and actions, improved extensibility of APIs, and reduced coupling between clients and servers.

There are numerous standards and conventions for defining structured hypermedia links in JSON payloads, including https://stateless.group/hal_specification.html[HAL (Hypertext Application Language)], JSON API, and https://github.com/kevinswiber/siren[Siren]. The following convention is takes some design cues from each of these, but is primarily inspired by https://github.com/levid-gc/paypal-api-standards/blob/master/api-style-guide.md#hypermedia[PayPal's convention].

////

PayPal's API conventions for hypermedia links align with the principles of HATEOAS (Hypermedia as the Engine of Application State). They use hypermedia links to make their APIs self-descriptive and guide clients on how to interact with resources. Here's a general overview of their approach:

rel: Indicates the relationship of the link to the current resource (e.g., "self" for the current resource, "next" for pagination).

href: Specifies the URL of the resource.

method: Defines the HTTP method (e.g., GET, POST) that can be used with the link.
