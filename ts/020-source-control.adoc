...
Source control SHOULD be tightly integrated with issue tracking. Using a combination of the two systems make it much easier to manage changes in source code and configuration. All-in-one development operations services like GitHub, GitLab, Azure DevOps, and Atlassian's software suite, provide deep integrations between source control and issue tracking out-of-the-box. See also *link:./019-issue-tracking.adoc[TS-19: Issue Tracking]*.

Source control is also the cornerstone of continuous integration and continuous delivery practices. Neither are really possible without a well-designed process for using source control systems. See also *link:./021-github-actions.adoc[TS-21: GitHub Actions]*.
...
* *Parallel development*: One of the primary use cases for source control systems is to support multiple parallel work streams on the same codebase. The source control workflow should not overly restrict this, but parallel development should not be entirely unbounded, either. A well-designed source control workflow should introduce just enough friction to avoid some of the risks associated with parallel development, notably the costs of merge conflicts.
...
* *Fast rollback*: Continuous deployment also requires fast reproducibility of _prior_ versions. If an incident occurs in production after a release, we MUST be able to rollback to the last known good version _as quickly as possible_, and with a high degree of confidence that the rollback will work and the prior version will still be stable. A goal of this source control workflow is to be able to recreate a system, or any prior version of it, at any time.
...
[IMPORTANT]
======
Some aspects of this objectives are covered by *link:./031-releasing.adoc[TS-31: Releasing]*.
======
...
----
$ git commit -m "maintenance: add readme"
----
...
|===
|Branch type |Naming convention |Role |Ownership |Lifespan |Mutable |Requirement level
...
|Epic
|`([version]/)epic/[issue_id]-[description]`
|Sub-task integrations for epic changes
|Group or sub-team
|Temporary
|No
|OPTIONAL
...
|Environment (or test)
|`([version]/)qa`, `([version]/)uat`, `([version]/)staging`
|Quality assurance
|Team
|Permanent
|No
...
An atomic commit is a small, self-contained, incremental change to the codebase that does not break the build or tests. A commit does not necessarily represent a "complete" feature or fix, but it does represent a small logical step toward that goal.
...
In each commit, tests SHOULD pass and the build SHOULD succeed. This means avoiding commits that "fix the tests that were broken in the previous commit". However, issue branches MAY be used to implement changes, and these MAY break the rules for commit messages, as long as the commits are eventually fixed up, for example by using squashing, `git commit --amend`, interactive rebasing, or other techniques for rewriting commit histories.
...
Small, incremental changes are easier to understand. They make it easier to track the history of the codebase and to identify the purpose of each change.
...
The smaller individual commits, the better. If your commits are too granular, you can always squash them together. But it is harder to do the opposite: splitting a large commit into smaller ones.
