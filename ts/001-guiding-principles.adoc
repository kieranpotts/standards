...
This technical standard provides a framework for _thinking_ about software development projects. It outlines key principles for software design and construction, which lead to improved outcomes.
...
Some software projects have stable requirements, like platform migrations that replace old systems with like-for-like features. But even these projects face uncertainty. At the very least, the software will need to be maintained: dependencies updated, newly discovered vulnerabilities patched, code and configuration refactored for compliance with new runtimes and standards. If software is not maintained, *software rot* sets in. The long-term cost of software maintenance is unknown.
