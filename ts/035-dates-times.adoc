...
Dates and times SHOULD be stored in UTC, and converted to the local time zone only when required for application output. An exception to this rule is when the local time zone is a critical part of the data and it is a requirement that the data is always represented in the original time zone.
