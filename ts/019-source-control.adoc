...
TODO: List recommended branch protection rules, which help to enforce the branching-and-merging strategy described below.

For example, you may want to enforce the the main line tracks are protected from direct commits (and therefore changes can be added only via pull requests), that these branches cannot be deleted, and that they must have a linear commit history (which means explicit merge commits, which have two or more parent commits, will not be allowed).
