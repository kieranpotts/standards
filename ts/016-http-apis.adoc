...
To indicate errors, the RECOMMENDED status codes are:

* `400 Bad Request` for client errors, which you should return if the message failed to validate against the expected schema.
...
* `500 Internal Server Error` for any scenario in which your application encounters an unexpected condition that prevents it from fulfilling the request.
...
* To treat `5**` status codes as errors, which will feed into your retry and dead-letter queue mechanisms.
