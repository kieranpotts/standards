...
// TODO: Extend table with more information about what is allowed for each method. Add safe and idempotency too. No body allowed for DELETE requests (many web servers just discard it; the standard say that the body of a DELETE request is allowed but has no semantics).
...
* https://smartbear.com/learn/api-design/[API Design] by SmartBear — An extensive collection of articles and resources all about web API design, by [Smartbear](https://smartbear.com/), a maker of tools for software developers.
