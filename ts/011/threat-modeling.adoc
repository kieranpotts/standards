...
Threat modeling is more of a process than a tool or an artifact. It works best when it is iterative, baked into the regular development process, rather than a one-time activity. Threat modeling sessions SHOULD begin during the design phase of a new software product. Thereafter, the threat assessments SHOULD be treated as living documents that are regularly revised as the system evolves, new threats emerge, or business requirements change.
...
It is RECOMMENDED to schedule threat modeling sessions at the break point between each iterative development/release cycle. Alternatively, threat modeling sessions can be conducted asynchronously, rather than at scheduled intervals. Either way, threat modeling sessions should be organized and facilitated by a security champion within each team.
...
Potential threats SHOULD be identified using a structured framework. The following standardized threat modeling frameworks are helpful. You may choose to use a combination of these frameworks, or a custom framework tailored to your own needs.
...
To identify potential vulnerabilities, analysts use either the STRIDE framework or a vulnerability database, such as the https://www.cve.org/[Common Vulnerabilities and Exposures (CVE)] database, the https://cwe.mitre.org/index.html[Common Weakness Enumeration (CWE)] list, https://capec.mitre.org/[Common Attack Pattern Enumerations and Classifications (CAPEC)], the https://nvd.nist.gov/vuln/search#/nvd/home?resultType=records[National Vulnerability Database (NVD)], or the https://cheatsheetseries.owasp.org/[OWASP Cheat Sheets] or https://owasp.org/Top10/[OWASP Top 10] lists from the Open Worldwide Application Security Project.
...
In threat modeling sessions, analysts systematically examine each system component against various categories of threats, to reveal potential vulnerabilities in the design. The following questions, grouped into the six STRIDE categories, serve as useful prompts for analysts.
