...
If all the script is doing is mostly calling other utilities, and if its doing very little data manipulation, then shell is probably a good technology choice for the problem.

However, if performance matters, there will probably be more optimum technology choices than shell. Likewise, if the logic include any non-trivial control flows, it may well be better to use a more structured language.

Rewrite shells scripts, in other languages if necessary, if they grow too complex. A single shell script that is more than 200 lines of code (excluding comments and included all sourced files) is a *code smell* – it _may_ be time for a rewrite in another language.
...
It is RECOMMENDED to use `set` at the top of scripts to set shell options, so that calling the script using `sh <script-name>` (without parameters) does not break its functionality.

== Execution and permissions

Scripts that are intended to executed directly, like commands, MUST be executable. This is achieved by setting the executable bit on the script file: `chmod +x <script-name>`.

Libraries – shell scripts that are intended only to be sourced by other scripts – MUST NOT be executable. This is achieved by removing the executable bit on the script file: `chmod -x <script-name>`.

SUID and SGID MUST NOT be applied to _any_ shell scripts, whether or not those scripts are executed directly. These permissions open up potential vulnerabilities. Programs with SUID (Set User ID) are run with the elevated permissions of the file owner, so increasing risk if the executed scripts are exploitable. And SGID (Set Group ID), when applied to a file, executes the file with the group permissions of the file rather than the user's current group. SGID is used to give group access to executables, but it can lead to unintended access if those groups are not managed carefully.
...
Use the following commands to group SUID and SGID from shell scripts:
...
The structure applies to a whole script, including all of its sourced files. This is also a useful structure for decomposing large scripts into smaller, more manageable files.
...
For libraries – shell scripts that are intended only for sourcing into other scripts, rather than direct execution – the shebang line is optional but RECOMMENDED. Including the shebang in all shell scripts, including sourced ones, provides clarity over which shell the script targets for compatibility.
