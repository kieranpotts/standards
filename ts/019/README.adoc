...
Different shells support extended non-POSIX options. For example, Bash supports `set -o pipefail`, which causes scripts to exit when any command within a pipeline fails. Without this option, `set -e` would exist only if the last command in the pipeline fails. This Bash option tends to be used as an enhancement to POSIX-compliant `-e` and `-u` options:
